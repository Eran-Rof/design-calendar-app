    ...
    <div style={S.app}>
      ...
      <div style={S.content}>
        ...
        {view === "timeline" && (() => {
          ...
          const filteredPOs = filterVendor === "All" ? posWithMs : posWithMs.filter(p => (p.VendorName ?? "") === filterVendor);
            ...
            <>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <select style={S.select} value={filterVendor} onChange={e => setFilterVendor(e.target.value)}>
                  ...
                  <div style={{ display: "flex", gap: 10, fontSize: 10, color: "#6B7280" }}>
                    {[["#10B981","Complete"],["#3B82F6","In Progress"],["#EF4444","Delayed"],["#4B5563","Not Started"]].map(([c,l]) => (
                      <span key={l} style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ width: 12, height: 8, borderRadius: 3, background: c }} />{l}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div style={{ background: "#1E293B", borderRadius: 12, border: "1px solid #334155", overflow: "hidden" }}>
                <div style={{ display: "flex" }}>
                  ...
                  <div style={{ width: LEFT_W, flexShrink: 0, zIndex: 3, background: "#1E293B" }}>
                    {/* Header cells */}
                    <div style={{ height: 28, background: "#0F172A", borderBottom: "1px solid #334155", display: "flex", alignItems: "center", padding: "0 12px" }}>
                      <span style={{ fontSize: 10, fontWeight: 700, color: "#6B7280", textTransform: "uppercase", letterSpacing: 1 }}>Month</span>
                    </div>
                    <div style={{ height: 24, background: "#0F172A", borderBottom: "1px solid #334155", display: "flex", alignItems: "center", padding: "0 12px" }}>
                      <span style={{ fontSize: 10, fontWeight: 600, color: "#6B7280", textTransform: "uppercase", letterSpacing: 0.5 }}>Week of</span>
                    </div>
                    {/* PO labels */}
                      ...
                      return (
                        <div key={poNum} onClick={() => { setDetailMode("milestones"); setNewNote(""); setSearch(""); setSelected(po); }}
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 10, color: "#94A3B8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{po.VendorName ?? ""}</div>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2, flexShrink: 0 }}>
                            <span style={{ fontSize: 10, color: pct === 100 ? "#10B981" : "#6B7280", fontWeight: 700, fontFamily: "monospace" }}>{pct}%</span>
                            <div style={{ width: 36, height: 3, borderRadius: 2, background: "#334155", overflow: "hidden" }}>
                      ...
                      <div style={{ height: 28, position: "relative", background: "#0F172A", borderBottom: "1px solid #334155" }}>
                        {monthSpans.map((ms, i) => (
                          <div key={i} style={{ position: "absolute", left: ms.left, width: ms.width, height: "100%", display: "flex", alignItems: "center", justifyContent: "center", borderRight: "1px solid #334155" }}>
                            <span style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", letterSpacing: 0.5 }}>{ms.label}</span>
                          </div>
                        ))}
                      </div>
                      {/* Week header row */}
                      <div style={{ height: 24, position: "relative", background: "#0F172A", borderBottom: "1px solid #334155" }}>
                        {weeks.map((w, i) => {
                          ...
                          return (
                            <div key={i} style={{ position: "absolute", left: w.offset, width: wWidth, height: "100%", display: "flex", alignItems: "center", justifyContent: "center", borderRight: "1px solid #1E293B", background: isThisWeek ? "#F59E0B15" : "transparent" }}>
                              <span style={{ fontSize: 9, color: isThisWeek ? "#F59E0B" : "#4B5563", fontWeight: isThisWeek ? 700 : 400 }}>
                            ...
                            {WIP_CATEGORIES.map((cat, catIdx) => {
                              ...
                              return (
                                <div key={cat} title={`${cat}: ${catDone}/${catActive} complete\n${catStart} → ${catEnd}`}
                                  ...
                                  {barW > 50 && <span style={{ fontSize: 7, color: "#fff", fontWeight: 700, paddingLeft: 4, whiteSpace: "nowrap", opacity: 0.9 }}>{cat}</span>}
                                </div>
                              );
                            })}
                          </div>
