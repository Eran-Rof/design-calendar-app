// Inline detail panel for a selected (sku, period). Renders below
// the reconciliation grid (NOT a drawer overlay) so the planner can
// keep the grid context visible while reading the breakdown — same
// no-drawer convention the request panel uses.
//
// Shows the full reconciliation: supply components, demand breakdown
// (customer + channel), allocation waterfall trace, applicable rules,
// and every recommendation the engine emitted for that grain.
...
export default function AllocationDetailPanel({
  ...
  return (
    <div style={{ ...S.card, marginTop: 12 }}>
      {/* Header bar — sku identity + close button. Sticky-feeling, but
          no overlay; this section flows in normal page layout. */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: `1px solid ${PAL.borderFaint}`, paddingBottom: 10, marginBottom: 12 }}>
        <div>
        ...
        <button style={S.btnGhost} onClick={onClose} title="Close detail">✕</button>
      </div>
      ...
      <div style={{ fontSize: 11, color: PAL.textMuted, marginTop: 4 }}>
        Total supply excludes ATS to avoid double-counting on-hand. Planned buys count only when the run flag is on.
      </div>
