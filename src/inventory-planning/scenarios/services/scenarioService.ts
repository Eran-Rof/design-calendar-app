...
export async function cloneBaseIntoSavedBuild(args: {
  ...
  onProgress?.("Creating snapshot run");
  const newRun = await wholesaleRepo.createPlanningRun({
    name: `[Saved] ${name}`,
  ...
  // Forecast rows — carry override / planned_buy / unit_cost so planner
  // edits survive in the snapshot.
  onProgress?.("Cloning forecast rows");
  const baseForecast = await wholesaleRepo.listForecast(baseRunId);
  if (baseForecast.length > 0) {
    await wholesaleRepo.upsertForecast(baseForecast.map((r) => ({
      planning_run_id: newRun.id,
      customer_id: r.customer_id,
      category_id: r.category_id,
      sku_id: r.sku_id,
  ...
  // Recommendations — saved so the snapshot view doesn't have to
  // re-run the rolling supply pass. New IDs auto-generated by Postgres.
  onProgress?.("Cloning recommendations");
  const baseRecs = await wholesaleRepo.listRecommendations(baseRunId);
  if (baseRecs.length > 0) {
    const recsToWrite = baseRecs.map((r) => {
      // Strip id/created_at — let the new run get fresh PKs.
  ...
  // TBD rows — planner-added stock-buy + auto-routed catch-alls.
  onProgress?.("Cloning TBD stock-buy rows");
  const baseTbd = await wholesaleRepo.listTbdRows(baseRunId);
  if (baseTbd.length > 0) {
    const tbdRows = baseTbd.map((t) => ({
      planning_run_id: newRun.id,
    ...
    await wholesaleRepo.bulkInsertTbdRows(tbdRows);
  }

  // Bucket buy aggregates — preserves the planner's collapsed-mode buy
  // qtys at category / sub-cat / customer rollups.
  onProgress?.("Cloning bucket buys");
  const baseBuckets = await wholesaleRepo.listBucketBuys(baseRunId);
  if (baseBuckets.length > 0) {
    await wholesaleRepo.bulkInsertBucketBuys(baseBuckets.map((b) => ({
      planning_run_id: newRun.id,
  ...
  onProgress?.("Recording snapshot metadata");
  const scenario = await scenarioRepo.createScenario({
    planning_run_id: newRun.id,
    scenario_name: name,
