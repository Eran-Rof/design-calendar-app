...
function OrderTypeManager({ orderTypes, setOrderTypes, isAdmin = false }: {
  ...
  function save() {
    ...
    if (!val) return;
    if (editing === "new") {
      setOrderTypes((s: string[]) => [...s, val]);
    } else {
      setOrderTypes((s: string[]) => s.map((x: string, i: number) => (i === editing ? val : x)));
    }
    setEditing(null);
    setForm("");
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {orderTypes.map((ot: string, i: number) => (
          <div key={i} style={{ ...S.card, display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => appConfirm("You are about to delete this order type. This action cannot be undone.", "Delete", () => setOrderTypes((arr: string[]) => arr.filter((_: any, j: number) => j !== i)))} style={{ padding: "5px 12px", borderRadius: 7, border: "1px solid #FCA5A5", background: "none", color: "#B91C1C", cursor: "pointer", fontFamily: "inherit", fontSize: 12 }}>Delete</button>
            </div>
          </div>
        ))}
      </div>
