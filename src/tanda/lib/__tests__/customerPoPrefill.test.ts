...
describe("buildSeedFromResolved PPK packs vs units", () => {
  ...
  const ppkMatrix = async () => ({ sizes: ["PPK24"], colors: ["Indigo"] });
  ...
  it("seeds the pack count directly when qty_is_packs is true (no division)", async () => {
    ...
    expect(sec).toBeTruthy();
    // 20 packs land on the PPK24 column as 20 (not 20÷24 = 1).
    expect(sec!.cells).toEqual([{ color: "Indigo", size: "PPK24", qty: 20, unit: "216" }]);
  ...
  it("converts units → cartons when qty_is_packs is false", async () => {
    ...
    expect(sec!.cells[0]).toMatchObject({ size: "PPK24", qty: 20 });
  });
