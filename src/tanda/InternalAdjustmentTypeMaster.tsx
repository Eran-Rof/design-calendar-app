// src/tanda/InternalAdjustmentTypeMaster.tsx
//
// Tangerine — Adjustment Type Master admin panel.
// List + search + active toggle + create + edit + hard-delete (rejected with
// reference detail if any inventory_adjustments rows still reference the type by
// name). Wraps /api/internal/adjustment-types and
// /api/internal/adjustment-types/:id.
//
// An adjustment type is an inventory-adjustment CATEGORY / reason (e.g.
// Shrinkage, Damage, Found, Cycle Count, Write-off, Return to Vendor). The
// Inventory Adjustments panel stores the chosen type NAME as free text on
// inventory_adjustments.adjustment_type — this master simply curates the
// picklist; there is no FK and it does NOT drive FIFO accounting.
...
const ADJ_TYPES_TABLE_KEY = "tangerine:adjustment_types:columns";
const ADJ_TYPE_COLUMNS: ColumnDef[] = [
  { key: "code",       label: "Code" },
  { key: "name",       label: "Name" },
...
export default function InternalAdjustmentTypeMaster() {
  const [rows, setRows] = useState<AdjustmentType[]>([]);
  ...
  const { getRowProps } = useRowClickEdit<AdjustmentType>({
    onRowClick: (r) => setEditing(r),
    onBeforeRowClick: (id) => setHighlightedId(id),
    ariaLabel: (r) => `Edit adjustment type ${r.code}`,
  });
  ...
  async function load() {
    ...
    try {
      ...
      const r = await fetch(`/api/internal/adjustment-types?${params.toString()}`);
      if (!r.ok) throw new Error((await r.json().catch(() => ({}))).error || `HTTP ${r.status}`);
      setRows(await r.json() as AdjustmentType[]);
    } catch (e: unknown) {
  ...
  async function del(s: AdjustmentType) {
    if (!(await confirmDialog(`Delete adjustment type ${s.code} (${s.name})?\nWill fail if any adjustment still references it — toggle is_active=false in that case.`))) return;
    try {
      const r = await fetch(`/api/internal/adjustment-types/${s.id}`, { method: "DELETE" });
      if (!r.ok) {
        const j = await r.json().catch(() => ({}));
        if (r.status === 409 && j.references) {
          notify(`Cannot delete — still referenced by ${j.references.adjustments} adjustment(s).\n\nReassign those adjustments first, or toggle is_active=false instead.`, "error");
          return;
        }
  ...
  return (
    <div style={{ color: C.text }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 16 }}>
        <h2 style={{ margin: 0, fontSize: 22 }}>Adjustment Types</h2>
        <button onClick={() => setAddOpen(true)} style={btnPrimary}>+ Add type</button>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <ExportButton
          ...
          filename="adjustment-types"
          sheetName="Adjustment Types"
          columns={[
        ...
        />
        <TablePrefsButton
          tableKey={ADJ_TYPES_TABLE_KEY}
          columns={ADJ_TYPE_COLUMNS}
      ...
      <div style={{ background: C.card, border: `1px solid ${C.cardBdr}`, borderRadius: 10, overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 240px)" }}>
        {loading ? (
          ...
          <div style={{ padding: 20, textAlign: "center", color: C.textMuted }}>
            No adjustment types found. Add one with &quot;+ Add type&quot; — or check &quot;Show inactive&quot;
      ...
      {addOpen && (
        <AdjustmentTypeFormModal
          mode="add"
          onClose={() => setAddOpen(false)}
          onSaved={() => { setAddOpen(false); void load(); }}
        />
      )}
      {editing && <AdjustmentTypeFormModal mode="edit" type={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); void load(); }} />}
    </div>
  );
}
...
function AdjustmentTypeFormModal({ mode, type, onClose, onSaved }: ModalProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      let method: string;
      if (mode === "add") {
        url = "/api/internal/adjustment-types";
        method = "POST";
      } else {
        url = `/api/internal/adjustment-types/${type!.id}`;
        method = "PATCH";
      }
