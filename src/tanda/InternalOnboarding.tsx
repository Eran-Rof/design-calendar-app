        ...
        {loading ? (
          ...
          return (
            ...
            >
              ...
              <div style={{ color: C.textSub }}>{new Date(row.expires_at).toLocaleDateString("en-GB", { month: "2-digit", day: "2-digit", year: "numeric" })} {new Date(row.expires_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
...
function EditInviteModal({ invite, onClose, onSaved }: { invite: InviteRow; onClose: () => void; onSaved: () => void }) {
  ...
  const fmtDateTime = (s: string | null) => {
    if (!s) return "—";
    const d = new Date(s);
    return `${d.toLocaleDateString("en-GB", { month: "2-digit", day: "2-digit", year: "numeric" })} ${d.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
  };
