// src/tanda/InternalSeasonMaster.tsx
//
// Tangerine — Season Master admin panel.
// List + search + active toggle + create + edit + hard-delete (rejected with
// reference detail if any style_master rows still reference the season by name).
// Wraps /api/internal/seasons and /api/internal/seasons/:id.
//
// A season is a named merchandising window (e.g. FW26, SS27, HOLIDAY26). Style
// Master stores the chosen season NAME as free text on style_master.season —
// this master simply curates the picklist; there is no FK.
...
const SEASONS_TABLE_KEY = "tangerine:seasons:columns";
...
export default function InternalSeasonMaster() {
  ...
  const { getRowProps } = useRowClickEdit<Season>({
    onRowClick: (r) => setEditing(r),
    onBeforeRowClick: (id) => setHighlightedId(id),
    ariaLabel: (r) => `Edit season ${r.code}`,
  });
  ...
  async function load() {
    ...
    try {
      ...
      const r = await fetch(`/api/internal/seasons?${params.toString()}`);
      if (!r.ok) throw new Error((await r.json().catch(() => ({}))).error || `HTTP ${r.status}`);
      setRows(await r.json() as Season[]);
    } catch (e: unknown) {
  ...
  async function del(s: Season) {
    if (!(await confirmDialog(`Delete season ${s.code} (${s.name})?\nWill fail if any style still references it — toggle is_active=false in that case.`))) return;
    try {
      const r = await fetch(`/api/internal/seasons/${s.id}`, { method: "DELETE" });
      if (!r.ok) {
  ...
  return (
    <div style={{ color: C.text }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 16 }}>
        <h2 style={{ margin: 0, fontSize: 22 }}>Seasons</h2>
        <button onClick={() => setAddOpen(true)} style={btnPrimary}>+ Add season</button>
      </div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.cardBdr}`, borderRadius: 10, overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 240px)" }}>
        {loading ? (
          ...
          <div style={{ padding: 20, textAlign: "center", color: C.textMuted }}>
            No seasons found. Add one with &quot;+ Add season&quot; — or check &quot;Show inactive&quot;
      ...
      {editing && <SeasonFormModal mode="edit" season={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); void load(); }} />}
    </div>
  );
}
...
function SeasonFormModal({ mode, season, onClose, onSaved }: ModalProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      let method: string;
      if (mode === "add") {
        url = "/api/internal/seasons";
        method = "POST";
      } else {
        url = `/api/internal/seasons/${season!.id}`;
        method = "PATCH";
      }
    ...
    >
      ...
      >
        <h3 style={{ margin: "0 0 16px", fontSize: 18 }}>
          {mode === "add" ? "Add season" : `Edit ${season!.code}`}
        </h3>
