// src/tanda/InternalTransferReasonMaster.tsx
//
// Tangerine — Transfer Reason Master admin panel.
// List + search + active toggle + create + edit + hard-delete (rejected with
// reference detail if any inventory_transfers rows still reference the reason by
// name). Wraps /api/internal/transfer-reasons and
// /api/internal/transfer-reasons/:id.
//
// A transfer reason is an inventory-transfer CATEGORY / reason (e.g.
// Replenishment, Rebalance, Damage Move, Return to Warehouse, Cycle-Count
// Correction). The Inventory Transfers panel captures the chosen reason NAME as
// free text on inventory_transfers.notes — this master simply curates the
// picklist; there is no FK and it does NOT drive any accounting.
...
const XFER_REASONS_TABLE_KEY = "tangerine:transfer_reasons:columns";
const XFER_REASON_COLUMNS: ColumnDef[] = [
  { key: "code",       label: "Code" },
  { key: "name",       label: "Name" },
...
export default function InternalTransferReasonMaster() {
  const [rows, setRows] = useState<TransferReason[]>([]);
  ...
  const { visibleColumns, toggleColumn, setAllVisible, resetToDefault } = useTablePrefs(
    XFER_REASONS_TABLE_KEY,
    XFER_REASON_COLUMNS,
  );
  const isVisible = (k: string): boolean => visibleColumns.has(k);

  const { getRowProps } = useRowClickEdit<TransferReason>({
    onRowClick: (r) => setEditing(r),
    onBeforeRowClick: (id) => setHighlightedId(id),
    ariaLabel: (r) => `Edit transfer reason ${r.code}`,
  });
  ...
  async function load() {
    ...
    try {
      ...
      const r = await fetch(`/api/internal/transfer-reasons?${params.toString()}`);
      if (!r.ok) throw new Error((await r.json().catch(() => ({}))).error || `HTTP ${r.status}`);
      setRows(await r.json() as TransferReason[]);
    } catch (e: unknown) {
  ...
  async function del(s: TransferReason) {
    if (!(await confirmDialog(`Delete transfer reason ${s.code} (${s.name})?\nWill fail if any transfer still references it — toggle is_active=false in that case.`))) return;
    try {
      const r = await fetch(`/api/internal/transfer-reasons/${s.id}`, { method: "DELETE" });
      if (!r.ok) {
        const j = await r.json().catch(() => ({}));
        if (r.status === 409 && j.references) {
          notify(`Cannot delete — still referenced by ${j.references.transfers} transfer(s).\n\nReassign those transfers first, or toggle is_active=false instead.`, "error");
          return;
        }
  ...
  return (
    <div style={{ color: C.text }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 16 }}>
        <h2 style={{ margin: 0, fontSize: 22 }}>Transfer Reasons</h2>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        />
        <TablePrefsButton
          tableKey={XFER_REASONS_TABLE_KEY}
          columns={XFER_REASON_COLUMNS}
      ...
      <div style={{ background: C.card, border: `1px solid ${C.cardBdr}`, borderRadius: 10, overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 240px)" }}>
        {loading ? (
          ...
          <div style={{ padding: 20, textAlign: "center", color: C.textMuted }}>
            No transfer reasons found. Add one with &quot;+ Add reason&quot; — or check &quot;Show inactive&quot;
      ...
      {addOpen && (
        <TransferReasonFormModal
          mode="add"
          onClose={() => setAddOpen(false)}
          onSaved={() => { setAddOpen(false); void load(); }}
        />
      )}
      {editing && <TransferReasonFormModal mode="edit" reason={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); void load(); }} />}
    </div>
  );
}
...
function TransferReasonFormModal({ mode, reason, onClose, onSaved }: ModalProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      let method: string;
      if (mode === "add") {
        url = "/api/internal/transfer-reasons";
        method = "POST";
      } else {
        url = `/api/internal/transfer-reasons/${reason!.id}`;
        method = "PATCH";
      }
    ...
    >
      ...
      >
        <h3 style={{ margin: "0 0 16px", fontSize: 18 }}>
          {mode === "add" ? "Add transfer reason" : `Edit ${reason!.code}`}
        </h3>
