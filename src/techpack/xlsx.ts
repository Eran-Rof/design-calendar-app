...
import { newWorkbook, addObjectGridSheet, renderStyledAoa, downloadExcelWorkbook, type ExcelJS } from "../shared/excelLogo";

const MATERIAL_HEADERS: Record<string, string> = {
  name: "Name", type: "Type", composition: "Composition", weight: "Weight",
  width: "Width", color: "Color", supplier: "Supplier", unitPrice: "Unit Price",
  moq: "MOQ", leadTime: "Lead Time", certifications: "Certifications", notes: "Notes",
};
...
export function buildSpecSheetWb(
  ...
  // ── Cell helpers (write into an AoA the shared renderer turns into ExcelJS) ──
  const aoa: any[][] = [];
  const merges: any[] = [];
  ...
  // ── Finalize — render the AoA onto an ExcelJS sheet with the logo ──────────
  const cols = [2.5, 6, 34, 8, ...sizes.map((_: any, i: number) => (i === baseSzIdx ? 9 : 7.5))];
  const rowHeights: number[] = [6, 22, 16, 18, 18, 18, 22];
  for (const row of sheet.rows) rowHeights.push(row.isSection ? 18 : 17);
  if (sheet.rows.length === 0) for (let i = 0; i < 8; i++) rowHeights.push(18);
  void totalCols; void r;

  const wb = newWorkbook();
  renderStyledAoa(wb, "Spec Sheet", aoa, { banner: { cols: totalCols }, merges, cols, rowHeights });
  return wb;
}

/** Trigger a browser download for a built (ExcelJS) workbook. */
export function xlsxDownload(wb: ExcelJS.Workbook, filename: string, showToast?: ToastFn): void {
  downloadExcelWorkbook(wb, filename).catch((e) => {
...
export function downloadMaterialsExcel(mats: Material[], showToast?: ToastFn): void {
  const rows = mats.map((m) => ({
    ...
    moq: m.moq, leadTime: m.leadTime, certifications: m.certifications.join(", "), notes: m.notes,
  }));
  const wb = newWorkbook();
  addObjectGridSheet(wb, "Materials", rows, {
    title: "Materials Library",
    currencyKeys: ["unitPrice"],
