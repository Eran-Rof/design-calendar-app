...
interface XoroSync {
  syncing: boolean;
  syncProgress: { step: string; pct: number } | null;
  syncError: string | null;
  syncFromXoro: () => Promise<void>;
}
...
export const NavBar: React.FC<NavBarProps> = ({
  ...
  exportToExcel, filtered, displayPeriods, atShip, xoroSync,
}) => (
  <nav style={S.nav}>
    <div style={S.navLeft}>
    ...
    <div style={S.navRight}>
      ...
      {xoroSync && (
        <button
          style={{ ...S.navBtn, background: "#1E40AF", border: "1px solid #1E3A8A", color: "#fff", fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6 }}
          onClick={xoroSync.syncFromXoro}
          disabled={xoroSync.syncing || uploadingFile}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 2v6h-6" /><path d="M3 12a9 9 0 0 1 15-6.7L21 8" />
            <path d="M3 22v-6h6" /><path d="M21 12a9 9 0 0 1-15 6.7L3 16" />
          </svg>
          {xoroSync.syncing ? (xoroSync.syncProgress?.step || "Syncing…") : "Sync Xoro"}
        </button>
      )}
