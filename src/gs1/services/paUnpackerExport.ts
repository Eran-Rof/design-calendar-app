...
function buildNotes(fileNames: string[]): XLSXStyle.WorkSheet {
  ...
  const rows: Row[] = [
    ...
    [""],
    ["Verification (runs automatically on every parse):"],
    ["  1. Channel totals  – Σ units per channel tie out to the PA-reported TOTALS row."],
    ["  2. Line rows       – Σ units per source row tie out to that row's TOTAL UNITS (col 3)."],
    ["  3. Color coverage  – every color in the sheet produces its own records (no merging)."],
    [""],
