...
-- 8 brands under ROF (PLM removed per CEO; append-only set). ROF = default.
-- MPL Epic + MPL Sun & Stone are the Macy's private-label brands. Their WS/EC
-- partitions + channel map are generated by the generic SELECT inserts below
-- (every non-PT brand gets the standard separate-WS/EC treatment).
INSERT INTO brand_master (entity_id, code, name, is_default, sort_order) VALUES
  ...
  (rof_entity_id(), 'MPLEPIC',     'MPL Epic',         false, 70),
  (rof_entity_id(), 'MPLSUNSTONE', 'MPL Sun & Stone',  false, 80)
