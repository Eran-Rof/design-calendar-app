-- 20260419300000_scorecard_accuracy_fix.sql
--
-- Fix: invoice_accuracy_pct was returning 100% for vendors with POs
-- but no invoices because three_way_match_view counts ALL po_line_items
-- (including 'pending' and 'awaiting_invoice' lines). Those lines have
-- no discrepancy, so `(line_count - 0) / line_count = 100%` fired even
-- when nothing had been invoiced.
--
-- New denominator: only invoicing-active lines — matched + discrepancy.
-- Gate on invoice_count > 0 so periods with zero invoices return NULL.
...
SELECT
  ...
  -- Accuracy = matched / (matched + discrepancy). Requires invoices; NULL
  -- if no invoicing activity has produced matched or discrepancy lines yet.
...
-- Same fix applied to the snapshot function.
