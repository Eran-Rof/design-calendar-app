-- #1923 -- sync po_dq_norm_color() with the JS COLOR_ABBR dictionary.
--
-- Adds BLEACH -> BLEACHED. This is a WORD-FORM fold rather than an abbreviation:
-- CEO 2026-07-23, "Blue Bleach and Blue Bleached are the same, use Blue Bleached".
-- Verified against the live catalog before folding -- 'Bleach' occurs in exactly
-- one colour string ("Blue Bleach", 7 SKUs, style RYB0991 only) and 'Bleached' in
-- one ("Blue Bleached", 27 SKUs over 5 styles), so it collapses that single pair
-- and touches nothing else.
--
-- Mirrors api/_lib/xoroLineMatch.js COLOR_ABBR. The two MUST move together or
-- v_po_data_quality groups PO colours differently from the importer and emits
-- phantom incomplete_size_coverage defects. Single-word fold, so both sides are
-- byte-identical here (see the separator-asymmetry note in the #1920 migration).
--
...
-- at query time, so CREATE OR REPLACE is safe and needs no reindex.
...
AS $function$
  SELECT COALESCE(string_agg(
    CASE t.tok
      ...
      -- WORD-FORM fold (not an abbreviation) -- see the header note.
      WHEN 'BLEACH'     THEN 'BLEACHED'
      ELSE t.tok
    END, '' ORDER BY t.ord), '')
