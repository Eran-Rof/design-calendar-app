-- v_po_data_quality — surfaces catalog/link data-quality defects on ACTIVE native
-- POs so the operator is WARNED instead of silently misled (the class of bug that
-- had POs rendering "Style not found" / blank matrices / "most in one size" while
-- the grid looked fine). One row per finding. Powers
--   GET /api/internal/purchase-orders/data-quality
-- and the PO grid "Data Quality" report + per-PO ⚠ badge.
--
-- Defect classes (severity):
--   orphan_style_code       (error) — style_code not in style_master → "Style not found"
--   unlinked_line           (error) — PO line has no SKU → no cost/sell/matrix
--   ppk_no_prepack_def      (warn)  — PPK style has no active prepack matrix → blank/explode
--   incomplete_size_coverage(warn)  — a color carries 1 size while siblings carry ≥3
...
with active_lines as (
  select p.id as po_id, p.po_number, l.id as line_id, l.inventory_item_id,
         im.style_code, im.color, im.size
