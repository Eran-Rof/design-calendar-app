...
## 2026-05-17 — ATS sales-history: store grain + margin at ingest, not at export

- **Question:** ATS export shows prepack PPK explosion in real reports — e.g. RYG1768PPK (pack_size 81) rendering TY=1,620 vs LY=20 for the same $4,800 sale. We added `pack_size` to `ip_item_master` thinking it would fix this, but the bug persisted. Where's the actual source?
- **Choice:** Treat `ip_sales_history_wholesale.qty` as the *raw* Xoro value (mixed grain by row) and add explicit normalized columns: `qty_grain ('unit'|'pack')`, `qty_units` (authoritative unit-grain qty), `unit_cost_at_sale` (per-unit cost snapshot at sync time), `margin_amount` (= `net_amount - qty_units * unit_cost_at_sale`), `margin_pct`. Sync handler classifies grain from the raw Item Number BEFORE `canonStyleColor` strips PPK tokens — if any segment contains a `PPK\d*` token AND `master.pack_size > 1`, grain = `pack` (multiply qty by pack_size for qty_units); otherwise `unit`. Backfill via a one-time `unit_price`-closest-to-master-cost heuristic. ATS export reads the new columns directly, dropping the `/qtyDiv` math + the master-cost recomputation that was the proximate source of the explosion.
- **Why:** `pack_size` alone is the conversion factor, not the direction. Xoro records prepack sales sometimes as packs and sometimes as units within the same SKU — there's no UOM column in InvoiceDetail to disambiguate, and `canonStyleColor` strips PPK from sku_code so post-hoc inference from stored data fails. Inferring grain ONCE at ingest (when we still have the raw Item Number) + persisting it eliminates an entire class of grain-confusion bugs at the readers. Same logic applies to margin: computing at sync time uses the cost-at-sale snapshot so historical margins stay stable under future cost-cascade updates, and avoids per-export per-unit cost math that previously sometimes used pack-grain `master.unit_cost` divided by wrong-grain sales qty.
- **Rejected alternatives:** (1) Continue dividing `qty/qtyDiv` at the export with a better heuristic — won't work because the per-row grain isn't recoverable from the canonicalized sku_code. (2) Pure `pack_size > 1` → `pack` at backfill — false-positive on legitimate unit-grain sales of prepack-style size variants. (3) Add `qty_units` without `qty_grain` — operator can't override per-row misclassifications without re-running the heuristic from scratch.
- **Migration posture:** `qty_units` stays nullable for now. `sync-invoices.js` (the nightly path) populates it on every new row. Two other write paths (`api/_handlers/xoro-sales-sync.js` + `src/inventory-planning/services/excelIngestService.ts`) still leave it NULL — downstream readers COALESCE to `qty`. Follow-up issue tracks updating those two paths before tightening to `NOT NULL`.
...
- **Idempotency notes:** Re-running `scripts/consolidate-api-imports.mjs` is a one-off — safe to re-run since the regex now never matches (paths already have the extra `../`).
- **2026-06-04 update — routing is now data-driven (manifest + codegen).** `scripts/generate-api-routes.mjs` was deleted. It named handlers `h${i}` by sorted index (so any regen renumbered everything → conflicts, hence the abandoned-generator / hand-append drift that caused the #945 duplicate-`h527` cold-start outage) and derived every pattern from the file path (so it could not express the ~31 routes that inject an `:id` the path can't encode, and it skipped `api/cron`). Replaced by `api/_handlers/routes.manifest.js` (hand-edited `[pattern, module]` data) + `scripts/gen-routes.mjs` (`npm run gen:routes`), which names imports from the **module path** (stable — never renumbers) and emits the table in **specificity order** (so manifest order is irrelevant and a literal route can't be shadowed by an `:id` sibling). Migration verified behaviorally identical: all 572 routes resolve to the same handler as before. CI gate: `gen:routes --check`.
...
## 2026-05-16 — ATS Ask AI phase 2: DB query tools + opt-in push to grid
- **Question:** Phase 1 only saw the visible grid. Cross-table questions ("how many Edge did Ross order June 2026 vs ship same period last year") need read access to ip_sales_history_wholesale, ip_open_sales_orders, ip_open_purchase_orders. How to expose without giving Claude raw SQL?
- **Choice:** Add 5 read-only allowlisted query tools — `find_customer`, `find_style`, `query_shipments`, `query_open_sos`, `query_open_pos` — each maps to a parameterised PostgREST call against the appropriate `ip_*` table. Handler runs a tool-use loop (cap 6 iterations) so Claude can resolve names → ids → run queries → answer. Per-query row scan capped at 5000; results aggregated to ≤50 groups before returning. Switched model from Haiku → Sonnet 4.6 since multi-step tool plans need stronger reasoning. Added `suggest_grid_view` terminal tool: when an answer naturally maps to a grid subset, Claude proposes a label + filter shape; the panel renders a green "Push to grid" button; user clicks to apply. No auto-mutation when answers come through DB-query path — explicit confirmation per product requirement.
- **Why:** Allowlisted tools cap the blast radius (no raw SQL, no string interpolation, no write tables exposed). Aggregation server-side keeps token cost predictable. Opt-in push respects the operator: the AI never reaches into the grid based on a cross-table inference; the human stays in control of what's on screen.
- **Rejected alternatives:** (1) Raw SQL tool — too risky (prompt injection, exfiltration, accidental cartesian joins). (2) Auto-push after a DB-derived answer — surprises the operator when the AI guesses the wrong subset. (3) Materialised views per question shape — premature; the per-table tools handle every shape we've seen so far.

## 2026-05-16 — ATS Ask AI phase 3: cross-app DB access via schema-allowlisted generic query
- **Question:** Phase 2 covered ATS hot-path tables only. The user wanted the AI to answer questions across PO WIP, Vendor Portal, Planning, and Design Calendar without hand-writing a tool per table.
- **Choice:** Built a schema registry (`api/_handlers/ai/_schema.js`) declaring every AI-readable table per domain with per-column flags (filterable / groupable / aggregatable / date / pii). Added four generic tools — `list_domains`, `list_tables`, `describe_table`, `query_table` — that consume the registry. `query_table` accepts a structured filter list, optional group_by + aggregations + date_range; the executor validates every column + op against the registry before issuing a PostgREST call. PII columns (banking, encrypted card data, encrypted ERP config) are excluded at the schema-registry layer: whole tables (banking_details, payments, virtual_cards card fields) are absent from the registry, and individual columns flagged pii are stripped from every response and rejected as filter/group_by targets. Bumped tool-use loop cap to 10 to accommodate the discover → describe → query chain. Specialised hot-path tools (find_customer / find_style / query_shipments / query_open_sos / query_open_pos) kept as-is.
- **Why:** One generic tool covers the long tail (40+ tables across 4 apps) without 40+ hand-written tool definitions. The registry doubles as documentation — adding a new table means a single config entry. The PII allowlist is enforced server-side at the executor, so prompt injection can't request a non-readable column. Per the operator-confirmed PII policy: contact emails / phone OK, invoice $ / payment status OK, customer pricing OK; bank account / routing / card data excluded.
- **Rejected alternatives:** (1) Hand-written specialized tool per table — 40+ tools, big tool list pollutes Claude's context, hard to maintain. (2) Raw SQL via Claude — prompt injection risk, can't enforce PII column gating, easy to construct expensive queries. (3) Schema introspection from pg_catalog at runtime — exposes ALL tables by default (opt-out vs opt-in is the wrong default for safety).
