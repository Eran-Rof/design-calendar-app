...
**Status:** ✅ COMPLETE. A (backend) #792 · B (admin UI: Price Lists + Promotions panels + customer assignment, h579–h584) #793 · C (internal Sales-Order line price auto-fill — resolve endpoint accepts a SKU `item_id`→style; prefills the unit price, editable, with a ↻ re-pull + source-list hint) #794. **AR-invoice line auto-fill #795** (same pattern as SO). Matrix-entry auto-fill remains a fast-follow.
