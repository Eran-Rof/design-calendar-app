...
| Item | Why | Detail |
|---|---|---|
| **By-size-cutover on-hand goes stale (`xoro_rest_size` layers)** | Snapshot On Hand overstated for cut-over styles still selling via Xoro | The nightly phantom-on-hand sync (`ONHAND_LAYER_SYNC=apply`, live 2026-07-01) fixes `opening_balance`-seeded styles but **skips `xoro_rest_size`** styles (by-size cutover) to preserve size detail. Those layers are ALSO stale — **Xoro sales don't deplete native layers** — so e.g. **RYB0412 reads 48,382 on-hand vs ATS 14,195**. ⚠️ **Existing by-size work in flight** (`feat/by-size-pilot-ryb0412`, `feat/by-size-batch-cutover`, worktree `C:/tmp/rof-pilot` — all ~4 weeks old, possibly dormant). **Before building, confirm no active agent owns this.** Then scope a dedicated fix: **deplete the per-size `xoro_rest_size` layers from Xoro sales history** (keeps size detail) OR re-run the by-size cutover from the current feed. Financial — signed-off, reversible. See [[project_phantom_opening_balance_onhand]]. |
