...
### 22.12.6 Tolerant ItemNumber fallback — feed rows the UPC spine can't tie (2026-07-22)

Both the layer true-up and the by-size snapshot resolve each Xoro-REST row to a SKU. Until now a row resolved via **only** the UPC spine (`upc_item_master`) or the private-label ItemNumber path; a row whose item was UPC-unmapped was **skipped entirely** — its layers were never trued **and** its snapshot row never written, so both sides froze against the live feed. The recurring offenders are **lowercase / mixed-case BasePartNumbers** (`ptya0092`, `PTYG0003lstd`) and **inseam-embedded** ones (`RYB186230` = style `RYB1862` + inseam 30), which never matched a UPC row.

`scripts/sync-onhand-spine.mjs` now adds a **fallback resolver** (`api/_lib/spineFallback.js`) that fires only after the UPC and private-label paths miss, in two **never-guess** tiers, taking the first hit:

1. **normcode** — the feed ItemNumber, normalized (UPPER + strip every non-alphanumeric), matched to **exactly one** catalog `sku_code` (a norm shared by two different SKUs is dropped, not guessed). Generalizes the private-label `normSku` match to the whole catalog.
2. **inseam-tuple** — peel the trailing 2-digit inseam off the BasePartNumber, resolve the parent style (`resolveStyleToken` from `api/_lib/xoroLineMatch.js`, so `RYB186230` → style `RYB1862` + inseam 30), then pick the **exactly-one** `(colour, size, inseam)` catalog row (`pickColorSizeMatch`, which tolerates Xoro↔catalog spelling: Gray↔Grey, Lt↔Light, `S`↔`SMALL`, numeric waist sizes). Zero **or** multiple candidates → skip.

A fallback-resolved SKU joins the spine-managed set: its layers are trued and its snapshot pruned/superseded exactly like a UPC/PL item. Rows that resolve to nothing stay **skipped and counted** — never invented; a genuine catalog gap (e.g. an inseam program with no size-30 SKU) remains a gap. The nightly log now prints per-tier coverage: `upc / pl / normcode / inseam-fallback / unresolved` (with the unmapped-unit count), so the tee shows exactly how much of the feed ties each night. **Sold-through is unchanged:** an item entirely absent from the feed is never fallback-resolved (there is nothing to key on) and stays with `retire-soldthrough.mjs`'s independent grain-based retirement — the two never double-act, because a fallback item present in the feed has its grain in the CSV and `retire-soldthrough` excludes it.

> **Scope note (the accuracy-exposure residual):** as of 2026-07-22 the ~$33k monitor exposure is **99.98% stale `xoro_onhand_sync` layers** — a superseded legacy grain=color sync source (7,310 u across 75 items) that the current spine (`xoro_rest_size` only) intentionally never touches and that double-counts stock now held per-size. The fallback closes genuine **resolution** gaps (it added coverage for the UPC-unmapped rows), but it cannot move that residual, which is a separate legacy-layer retirement, not a resolution problem.
