...
**Closing the equal-total gap (#1909).** A shape neither pass above could reach: an **aggregate-only** historical invoice (a single styleless "Historical line" from the manual backfill) whose booked total *already equals* the CSV and the GL to the cent. #1898's enrichment skips it (the aggregate has no style/colour to group-match against the CSV), and the reprice pass skips it too (its total is "already correct"). With the CEO's confirmation that the delivered Invoice Detail CSVs are the **full export**, `reprice-ar-invoices-to-csv.mjs --fill-equal` targets exactly this residue: invoices with **zero size-grain lines** whose `|booked − CSV| ≤ 5¢` **and** `|CSV − GL AR| ≤ 5¢`, rebuilding their content to the CSV under the same machinery. Because booked = CSV = GL going in, the header total does not move **at all** (every period Δ = $0.00) — this is a pure content fill: 1 styleless line becomes N per-size lines, `cogs_cents` redistributed by qty, matching `ip_sales_history_wholesale` rows re-exploded to size grain. Touched lines carry `notes='[ar-size-fill-equal …]'`. The only unfixable residue inside the gate is invoices whose CSV rows carry no garment style at all (chargeback-reversal lines etc.) — skipped as `unresolved_style`. Invoices absent from the full export (the ~1,050 Sep–Dec 2024 `customer_invoice_historical` backfill invoices that were never in Xoro — verified: no `accrual_je_id`, no CSV rows) can never be sized from any Xoro source and remain honest style + colour rollups permanently.
