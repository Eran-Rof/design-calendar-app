...
**Display toggles.** **Hide Zeros** (default on) drops zero-total color rows within each style; **Totals only** (formerly *Hide sizes*, default off) drops **all per-size columns**, leaving the non-size columns (Color, Total, Avg Cost, Total Cost, Last Received) for a compact roll-up — this is now the **same per-user "Totals only" pref shared across every size matrix** in the suite (SO / PO / AR / PO detail / ATS), so flipping it here follows you everywhere; **Explode** folds PPK packs on-hand into sized eaches via the Prepack Matrix master; **By Inseam** splits each color into per-inseam rows with subtotals (shown only when the scale carries inseams). The **Warehouse** dropdown narrows on-hand to one warehouse (the on-hand *location*, per the house Warehouse-vs-Store rule) — on the Snapshot it constrains the **On Hand** (and ATS on-hand) columns to that warehouse's layers; the other lifecycle columns (Allocated / SO / PO / Sold / Purchased) are not warehouse-grained.

> **Empty size-column collapse (all matrix views, 2026-07-02; extended 2026-07-15; default flipped ON 2026-07-21).** The all-zero size columns *before* the first and *after* the last sized column are **now hidden by default** — every grid opens already collapsed to the size range actually in stock (mid-range zero sizes stay visible). The **first size column with quantity is green and clickable**: click it to **show all** columns, click again to re-hide; a leading/trailing **⋯** marks the hidden columns. The choice is a **per-user pref shared across every matrix surface** (persisted, so it follows you between the Inventory Matrix, SO/PO/AR grids, PO detail, the PO popover and the vendor portal). This works on the **single-style matrix AND every per-style block in the brand / multi-style view** — each block collapses independently (clicking one block's green header does not touch the others). It is the same green-first-column collapse the **Sales-Order and Purchase-Order size grids** use, plus the **PO detail "Item Matrix" tab**, the **PO grid row-detail (▸ expander) matrix**, the right-click **PO matrix popover**, and the **vendor-portal PO matrix** — all share the one `computeSizeCollapse` helper. The **Hide sizes** toggle (which drops every size column) and the collapse are independent; **exports always carry the full size set**, never the collapsed range.
...
> **Sold / Purchased drill popups.** Clicking a **Sold** or **Purchased** quantity opens a drill popup. Every column header in the popup sorts (▲/▼). The **Sold** popup adds a **Store** filter and a **Collapse: Invoice** toggle (rolls the rows up onto the invoice number); the **Purchased** popup adds a **Collapse: Bill** toggle (rolls onto the bill number) alongside the existing click-a-colour filter. Invoice numbers (AR) and bill Ref #s (AP) open a **full doc popup** (header + lines) with **✎ Edit (new tab)** → the AR Invoice / AP Bill editor. The drill popups clear the NavDrawer so they never slide under the menu on a narrow window. **The Purchased column counts the Xoro *receipts* feed where it exists and only falls back to the AP *vendor-bill* quantity for colours the receipts feed doesn't cover — so an Xoro PO carrying both is never double-counted; the drill lists whichever feed the column used (receipts, else bills). See chapter 28 → Inventory Snapshot for the full rule.**
