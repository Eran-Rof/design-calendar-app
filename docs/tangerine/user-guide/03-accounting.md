...
>
> **Operationally:** the mirror is kept current nightly in **two** steps. First the Xoro GL **sync** refreshes the staging table `xoro_gl_transactions` (`rest_gl_sync.py` → `/api/xoro/sync-gl`). Then, at **08:30 UTC Mon–Fri**, the **nightly GL mirror poster** cron (`/api/cron/xoro-gl-mirror-post`) posts every staged transaction not yet mirrored **for the current open month**, one `xoro_gl_mirror` JE per Xoro `TxnId`, posted the **same way the one-time rebuild did** — a direct double-entry insert at `posted`, byte-for-byte identical to the rebuild's JEs — while still stamping a **T11 audit reason** on every entry. (It posts direct rather than through the normal `gl_post_journal_entry` engine on purpose: factored-invoice mirror lines hit **1107 Accounts Receivable – Factor**, a _control account_, and the engine's guard rejects control-account lines that carry no customer/vendor **subledger**. That guard protects **native** postings; a 1:1 Xoro GL mirror has no subledger and must match Xoro/the rebuild exactly — so the poster runs as a `SECURITY DEFINER` function that briefly disables that one guard for its own insert, exactly as the rebuild did, keeping the audit trail and the period-lock backstop on the whole time. Without this, every factored invoice — a large share of wholesale — would fail to post.) It is **idempotent** (a `TxnId` already mirrored is skipped) and **self-healing** (a day missed for any reason is caught up the next run). It **aborts without posting anything** — and drops a breadcrumb into the daily _app-errors digest_ — if the open-month period isn't `open`, the staging feed is **stale** (no fresh sync), or any candidate transaction has an **unmapped account** or doesn't balance to the penny; that last case means someone must curate `xoro_account_map` (`scripts/build-xoro-account-map.mjs`) before the month can post cleanly. The posting logic lives in one reviewed place — SQL function `xoro_gl_mirror_post_open_month()` (migrations `20269000000000` + the control-account fix `20270500000000`). Native Tangerine JEs (operator-entered) still post through the normal Journal Entry panel below and are validated by the posting guards — the rebuild deleted only the superseded bottom-up reconstructions, never a native entry.
>
> _History note:_ this automation (CEO-approved 2026-07-21) closed a real gap — between the one-time 2026-07-13 rebuild and this cron there was **no** incremental poster, so 07-13…07-20 revenue (~$789K) sat staged-but-unposted and July GL revenue read **$1.10M** against Xoro's **~$1.9M** until it was re-posted. The "Open-period lag" recon category below is now genuinely _overnight_ lag rather than an open-ended hole.
