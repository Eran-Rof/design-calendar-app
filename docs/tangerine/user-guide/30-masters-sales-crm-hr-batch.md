...
**Scorecard fed by mirrored AR history + de-duplicated customer picker (2026-07-15).** The Customer Scorecard reads AR straight from the Xoro/Excel-mirrored `ar_invoices` (all invoice kinds, including `customer_invoice_historical`) for the selected customer, so a customer's balance, invoices, gender/period breakdowns reflect the full imported history. Two data issues that made it look empty were fixed:

- **Duplicate customer rows merged.** The import had created, for a few accounts, an ALL-CAPS machine-named row (e.g. `AMAZON FBM`, import ref `EXCEL:AMAZONFBM`) that the AR mirror attached invoices to, *and* a proper-cased row (`Amazon FBM`, `CUST-00008`) that carried the sales orders. Opening the scorecard for the proper customer showed no mirrored AR. These duplicates are now **merged into the proper-cased row** (all invoices/receipts/SOs repointed) and the ALL-CAPS row is retired, so the scorecard shows the complete picture and the picker lists each customer once.
- **Picker shows the proper name only.** The customer search no longer shows the legacy all-caps, no-space import identifiers (`EXCEL:MACYS`, `ATS:…`). Each customer appears as its internal `CUST-NNNNN` code and proper name (or just the name when no `CUST` code is assigned). You can still type an old import ref to find the customer; it is matched but not displayed.
