...
> **Colour spelling equivalences.** Just as with sizes (above), colour matching treats abbreviated and spelled-out colours as the *same colour*, so a PO, invoice or Xoro import line spelled the short way still lands on the right SKU. Folded pairs include `Lt`/`Lite` ≡ `Light`, `Dk`/`Drk` ≡ `Dark`, `Md`/`Med`/`Medm` ≡ `Medium`, `Blk`/`Blck` ≡ `Black`, `Gry`/`Gray` ≡ `Grey` (British spelling is the house standard), `Hthr` ≡ `Heather`, `Chrcl` ≡ `Charcoal`, `Nvy` ≡ `Navy`, `Wsh` ≡ `Wash`, `Cam` ≡ `Camo`, `Cbo` ≡ `Combo`, `Vtg` ≡ `Vintage`, `Oyst` ≡ `Oyster`, `Msty` ≡ `Misty`, `Plms` ≡ `Palms` and `Slt` ≡ `Slate`.
>
