...
> **Colour spelling equivalences.** Just as with sizes (above), colour matching treats abbreviated and spelled-out colours as the *same colour*, so a PO, invoice or Xoro import line spelled the short way still lands on the right SKU. Folded pairs include `Lt`/`Lite` ≡ `Light`, `Dk`/`Drk` ≡ `Dark`, `Md`/`Med`/`Medm` ≡ `Medium`, `Blk`/`Blck` ≡ `Black`, `Gry`/`Gray` ≡ `Grey` (British spelling is the house standard), `Hthr` ≡ `Heather`, `Chrcl` ≡ `Charcoal`, `Nvy` ≡ `Navy`, `Wsh` ≡ `Wash`, `Cam` ≡ `Camo`, `Cbo` ≡ `Combo`, `Vtg` ≡ `Vintage`, `Oyst` ≡ `Oyster`, `Msty` ≡ `Misty`, `Plms` ≡ `Palms`, `Slt` ≡ `Slate`, `Gd` ≡ `Garment Dyed` and `Bkb` ≡ `Black Black`.
>
> One upstream feed also writes colours with **every separator stripped**, so `Medium Blue` arrives as the single word `Mdblue`. Those glued spellings are folded too — `Dkblue`, `Mdblue`/`Medblue`/`Mdblu`/`Medblu`, `Ltblue`, `Ltwash`, `Mdwash`/`Medwash`, `Mdltwash`/`Medltwash`, `Ltgray`/`Ltgrey` and `Ltbrown` — so `Medusa-Dkblue` and `Medusa - Dark Blue` are recognised as one colour rather than two.
>
