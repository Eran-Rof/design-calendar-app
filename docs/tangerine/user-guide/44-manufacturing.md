...
1. **New build** — search and pick the **finished style** (the picker lists **base styles** — one row per style, code + name — not per-size SKUs), then **plan the run by size**: a color × size matrix (the same grid as SO/PO entry) lets you enter the quantity per size right away, and the **target quantity is the matrix total**. (If the style has no size scale, you just enter a total.) Creates a *draft*. **Add a style on the fly (item 1):** if the style isn't on file, click **+ New style** next to the picker — a popup creates the **Style Master record with a chosen size scale** (plus a first SKU) and selects it. **Admins only** (signed-in users); a non-admin sees a warning. *The new style still needs an active **BOM** (Master Data → BOM) before you can Release the build.*
