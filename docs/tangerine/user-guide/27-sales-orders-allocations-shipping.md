...
|---|---|---|
| Customer | **yes (to add styles)** | `SearchableSelect` over Customer Master. Selecting a customer **auto-sets Channel** from the customer — a **Shopify**-named customer ⇒ **DTC**, everyone else ⇒ **Wholesale** (matched by `channel_master` code; you can still change it) — and **pre-fills the Ship-to address** from the customer's default location (see Ship-to). Brand seeds from the customer default but is then **overridden by the selected style's brand** (see Brand below). **Add on the fly (item 8):** if you type a name that isn't on file, a **"+ Add customer '<name>'"** row appears in the dropdown — click it to open the Add-customer popup **pre-filled with that name**; on Save the new customer is selected here, and a **"complete the customer info"** reminder is sent to your notifications (the short form omits terms / GL routing / addresses). **Item 14:** on-the-fly customers get an auto `CUST-NNNNN` code as their `customer_code` (it's required), so the add no longer fails with a not-null error. |
...
| Cancel date | optional | **Item 15:** can't be **earlier than the Start ship date** — an inline warning shows and the save is blocked until it's fixed. |
...
| **Warehouse** | **yes (to add styles)** | `sales_orders.sale_store` — the order's **warehouse** (Xoro `SaleStoreName`, e.g. ROF Main / ROF - ECOM / Psycho Tuna). Tangerine has **warehouses + brands, no sales stores**, so this field reads *Warehouse*. **Required** to add styles. Drives the **Warehouse filter** on the SO list. Imported SOs are backfilled from the Xoro mirror. *(The column is still named `sale_store` internally; the value is the warehouse name.)* |
...
> **Warehouse filter (operator item 5).** A **Warehouse** dropdown lets you scope the list to one warehouse (`sales_orders.sale_store`, the Xoro `SaleStoreName`: ROF Main / ROF - ECOM / Psycho Tuna / Prebook - Psycho Tuna). Tangerine has **warehouses + brands, no sales stores**, so this reads *Warehouse*. "All warehouses" = no scope. The list comes from `GET /api/internal/sales-orders?facet=stores` (distinct values via the `distinct_so_sale_stores` function). Imported SOs were backfilled from the Xoro mirror; set the warehouse on app-created SOs via the **Warehouse** field in the order header. The all-field search runs in the `search_sales_orders` SQL function, so it spans the entire order book.
