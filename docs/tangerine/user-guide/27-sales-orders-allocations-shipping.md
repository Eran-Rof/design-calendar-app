...
| Ship-to location | optional at entry | The customer's `customer_locations` (stores / DCs). Re-fetched when customer is selected. **If the customer has exactly one location it is auto-selected.** If they have multiple, a dropdown appears — pick one or leave blank and assign later. At allocation time, if the SO still has no location and the customer has more than one, you will be prompted to assign one before proceeding. EDI-transmitted SOs always arrive pre-filled. Locations are managed in **Customer Master → Locations tab**; the list is seeded nightly from Xoro SO history via `rest_customer_locations_sync.py`. |
