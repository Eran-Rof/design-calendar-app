...
| **Customer PO #** | **yes (to add styles)** | `sales_orders.customer_po` — the buyer's own purchase-order reference. One of the four **add-styles prerequisites** (Customer · Ship-to warehouse · Customer PO # · Fulfillment source). **Item 16:** the **Add style / + Add non-matrix line** buttons now stay **visible**; clicking one before the prerequisites are filled shows a specific warning (e.g. *"Customer PO must be populated"*, *"Ship-to warehouse must be populated"*) instead of being hidden. A ⚠️ banner still lists what's missing. Free text. This is also the field the **🤖 Upload customer PO** AI flow fills in. |
