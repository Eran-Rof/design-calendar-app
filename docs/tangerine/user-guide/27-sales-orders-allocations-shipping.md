...
1. **➕ Add style (matrix)** — pick a style; it loads an editable **color × size (× inseam) grid** (the same `EditableSizeMatrix` the Inventory Matrix uses) where you type ordered quantities straight into the cells, with a per-row **Unit $** and a "set all rows" bulk field. The columns end with **Total** (row units) → **Unit $** → **Total $** (the extended line amount, units × Unit $), with a grand **Total $** in the footer. Unit prices **snap to two decimals** when you tab out of the field. Each **qty cell auto-widens to the digits you type**, so 4- and 5-digit quantities stay fully visible (the same on the Purchase Order matrix). Add more styles to stack more grids. The grids ARE the order — there is no separate "add to order" step.
   ...
   - **Collapse empty size columns (now ON by default)** — once any cell carries a quantity the **first size column header turns green** and becomes clickable. The all-zero size columns before the first sized column and after the last are **hidden by default** — so a wide scale (e.g. a 14-size denim waist run) opens already collapsed to just the sizes you're actually ordering. Mid-range zero sizes stay visible. A `⋯` marker shows where columns are hidden; click the green header to **show them all**, click again to re-hide. The preference is **remembered per user and shared across every size matrix** in the suite. (Available on both SO and PO size matrices.)
   - **Totals only** — a compact **✓ Totals only** toggle above the grid hides **every** per-size column and shows just the per-colorway totals (Color, Total qty, Unit $ / Total $). The totals are identical to the full grid's (nothing is recomputed) — a quick way to review order value without the size spread. Remembered per user and shared across every matrix surface; toggle off to bring the sizes back.
...
- Above the line matrix, click **Show line PO** (a toggle next to *Show lots* / *Show images*). Every style's matrix gains a **Customer PO** column with **one editable field per color row**, each **pre-filled with the header Customer PO** (a *set all* box in the column header stamps every color at once). The fields are always editable.
- Leave a color on the header PO and it stays on this order. **Change a color's PO to a different number** and its field turns **amber** (it will split).
- **On save**, for each distinct new PO, Tangerine **auto-creates a new sales order**: it copies **all of this order's header information** (customer, ship-to, brand, channel, dates, payment terms, buyer, warehouse, fulfillment source, factor fields), carries **only the color rows that share that PO** (rows with the same new PO — across any styles — are grouped onto one SO), is **saved and confirmed automatically** (its SO number is assigned immediately), and its **Notes** get the line **“Auto created due to new Customer PO.”**
- The order you were editing keeps the header PO and the rows that matched it. **At least one row must keep the header PO** — if you reassign *every* row away, save is blocked with a prompt (change one back, or update the header PO instead).
- A **summary dialog** then lists every new SO (number · PO · rows · qty) with an **Open** button each (opens in a new tab) plus **Open all**. Click **Done** to return to the list.

*This is a Sales-Order-only convenience; it needs no extra setup and adds no line-level PO column in the database — each resulting SO simply carries its own PO in the header. Non-matrix (flat) lines always stay on the parent order.*
...
>
> *Each tick refetches the list, and only the **latest** request's result is shown — so quickly narrowing to a single status (e.g. unticking the defaults and ticking **cancelled**) lands on exactly that status instead of momentarily flashing it and then reverting to a slower earlier response.*
...
The demand rows are grouped under a **per-SO sub-header** showing **SO # · Customer · Start Ship · Cancel**. The **SO #** is a link (shown in blue with a dotted underline): clicking it jumps to **🛒 Sales Orders** focused on that order — `?m=sales_orders&so=<SO#>` seeds the SO search box, so you land pre-filtered to it. This is the reverse of the SO modal's **📊 Allocations** drill, which brings you here focused on that same order.
