...
25. [Parallel-Run Reconciliation (P9 — Tangerine ⇄ Xoro per domain)](ch25-parallel-run-reconciliation.md) — **P9 COMPLETE (2026-05-29)** — 5 reconciliation engines (AP/AR/Cash/GL/Inventory) + dashboard UI + weekly Monday cron + variance notifications + close pre-flight extension (soft-block parallel / hard-block solo) + per-domain cutover automation (60-day clean window). Operator-locked thresholds. `🔁 Parallel Run` top-nav group with status grid + variance side panel + cutover history table.
