...
- **Avg cost** — the **qty-weighted average STANDARD (catalog) cost** across the PO's lines, from **`ip_item_avg_cost`** keyed by SKU (exact then loose match) — the *same* cost source the Inventory Snapshot and the Sales Orders grid use. This is what the item *normally* costs, not what this PO paid.
