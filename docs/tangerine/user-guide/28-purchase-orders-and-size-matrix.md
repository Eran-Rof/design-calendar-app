...
- **Totals** (Snapshot only, the **Qty / $ Cost / $ Retail** selector on the controls row) — modelled on the ATS totals view. Pick a mode to show a totals strip **above the column headers** summing every quantity column: **Qty** = unit counts, **$ Cost** = quantity × **avg unit cost**, **$ Retail** = quantity × **avg SO sale price** (#1445). Click the active mode again to turn totals off. The strip is sticky and reflects the rows currently shown (after Hide Zeros / Collapse / Merge PPK), so the totals always match the visible table (#1443).
