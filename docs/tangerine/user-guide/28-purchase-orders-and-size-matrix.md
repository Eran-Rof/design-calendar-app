...
1a. **Create from a Sales Order** (new PO) — the **📋 Create from Sales Order** button (above the matrix) opens a dynamic SO search. Pick an SO and its **styles / colors / sizes / quantities** fill the PO matrix, and the customer / brand / channel / dates carry over to the header. **Only qualifying SOs are listed** — orders already **allocated** or **invoiced** are excluded (their stock is committed / billed, so a PO from them is blocked; selecting one anyway is refused with a warning). **Unit costs stay blank** (the SO holds selling prices, not costs — set them by hand or via *Get PO price*). The PO records `sales_order_id` for traceability.
