...
- **Upload** reads **every sheet** and is section-aware (title / `INNER PACK` band / blank / legend rows skipped; a `PPK Style Code` header re-establishes the columns). It accepts the **inner-pack** format (above), the **long** format (`…| Size | Inner Pack Qty | Qty Per Box`), and **legacy wide** (paired `<size> Inner`/`<size> Box`, or a plain `<size>` = carton units), in `.xlsx` **or `.csv`**. The **+ Add matrix / Edit** modal builds the composition as a **per-size grid** (no typed syntax): one row per size with an **Inner Packs** and a **Box Qty** input, a single **Units / Inner Pack** field (Box Qty auto-fills = Inner Packs × Units/Inner Pack, still overridable), live **Inner-pack** and **Carton** totals, and a **warning when the carton total ≠ the pack token's qty** (e.g. PPK24 → must sum to 24). Add/remove sizes inline; **+ Create** from the "needs a matrix" list pre-loads the size columns. Both Inner Packs and Box Qty are saved per size, so editing a box quantity no longer drops the inner-pack breakdown.
