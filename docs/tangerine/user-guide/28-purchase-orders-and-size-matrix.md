...
### Why the nightly import stopped minting colourless "bare-shell" catalog rows (fixed)

Some Xoro lines carry an `ItemNumber` that is just a **bare style code** — `RYB1878`, or a prepack token like `RYO0659FPPPK` — with **no colour segment** (and, for a sales line, only a trailing size such as `RYB1878-30`). When the nightly sales / planning sync rolled such a line up to style+colour grain, the size fell off and it was left with a bare style token; the auto-stub then created an `ip_item_master` row whose `sku_code` **equals the style code**, with **`color` NULL and `size` NULL** — a junk **"bare shell."** These rows carry no colour, so (like the collapse case above) they pollute the catalog and can't be placed on any colour × size matrix; they were also getting **re-created every night** even after being deleted by hand.

The sync stub-create paths (`api/_handlers/xoro-sales-sync.js`, `api/_lib/planning-sync.js`) now **suppress** any would-be stub that has **no colour AND whose `sku_code` carries no colour segment** (the shared `isBareShellItemRow` / `dropBareShellRows` guard in `api/_lib/sku-canon.js`). Such rows are **not inserted** — they're counted in the sync's response as `bare_shells_suppressed: N` instead. Colour-bearing stubs (the normal `STYLE-COLOR` rollups and `STYLE-COLOR-SIZE` size-grain rows) are unaffected, and the six intentional colourless bookkeeping SKUs (`ADMINISTRATIVE`, `DEFAULTSKU`, `MISCELLANEOUSCHARGE`, and the three chargeback-reversal placeholders) are created by a different path and left untouched. If a genuinely new style needs a catalog row, add it through **Style Master / Item Master** (which set the colour) rather than relying on a colourless order line.
