...
- **Download template** produces a **wide paired-column matrix**: one row per PPK style, and **two columns per size** — `<size> Inner` (inner packs) and `<size> Box` (carton units of that size). The `Box` cells sum to the row's **Carton Total** (the `PPKnn` token). The template ships one filled example (`RYB059430PPK` / Edge Slim / PPK24).
- **⬇ Download all PPK** fetches every PPK style that still lacks a matrix (from `v_prepack_ppk_needed`, via `GET /api/internal/prepack-matrices/needed`) and writes one workbook **grouped into a section per size scale**, paired Inner/Box columns, cells blank to fill — names already populated **from the style master** (never guessed). Fill it in and upload the whole file in one go.
- **Upload** accepts the **wide paired** matrix (`<size> Inner` / `<size> Box`), the older single-column wide matrix (a plain `<size>` header = its Box), **and** the long (`…| Size | Inner Pack Qty | Qty Per Box`) layout, in `.xlsx` **or `.csv`**. The parser is **section-aware** — `#`-comment and blank rows are skipped and a repeated `PPK Style Code` header re-establishes the size columns — so the bulk file with one block per size scale imports in a single upload. The add/edit modal's composition field (`size:innerPacks:qtyPerBox`, e.g. `32:2:6`) is the manual equivalent.
