...
- **Export** reflects the **live filtered set** (all rows matching the active filters + Hide Zeros), or all rows if unfiltered — the `Export (n)` badge matches what's on screen. The matrix-view export only appears in **OH matrices**, not the Snapshot.
