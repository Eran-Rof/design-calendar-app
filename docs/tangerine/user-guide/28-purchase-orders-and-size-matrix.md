...
- **Per-column Avg Sale / Avg Cost basis on the Totals strip (#1800).** Each quantity column is valued from *its own* credible source, so the strip's **Avg Sale**, **Avg Cost** and **Avg Mrgn** describe the units in *that* column rather than one blanket price:
  ...
  - **PPK rows inherit the base garment's wholesale price (#1804).** A prepack (`…PPK`) is just N eaches of the **same** garment, so its per-each **Avg Sale** = the **base style's** qty-weighted SO price (resolved per base-style-code + colour, even when the base isn't on the current page), *not* the PPK's own pack-grain SO lines. Those pack lines are sparse and depend on a correct pack count — e.g. `RYB0412PPK-LTBROWN` is tagged **PPK24** but its pack SO'd at **$117/pack**, so ÷24 = **$4.88/each** vs the base's real **$7.13**; on 16k ATS-incl units that dragged the strip's Avg Sale to **$6.76**. Inheriting the base price fixes ATS-incl to **~$7.22** and pulls in PPK colours that had *no* pack SO price at all (previously excluded). Falls back to the PPK's own price only when the base has none. The **Sold** column is unaffected (PPK styles have no sales of their own; Sold already shows the real realized price). Resolution added to `api/_handlers/internal/inventory-snapshot.js` (`sale_price_cents` override on PPK rows).
