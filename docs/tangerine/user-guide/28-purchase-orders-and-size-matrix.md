...
- **By Inseam** (PR #1096) — appears **only when the picked style's size scale carries inseams** (§28.2). Toggling it ON splits each color into **one row per inseam** (ordered by the scale's inseam order, with any stray SKU inseams appended) and adds a **per-color Subtotal row** rolling up all inseams for that style + color. A new **Inseam** column (replacing the Rise column in this mode) shows each row's inseam (e.g. `32"`). The Export mirrors the on-screen grid — an Inseam column plus a `{color} — subtotal` row after each color group. This is the **reference implementation** of the inseam matrix rule; the same row-per-inseam + subtotal pattern is being swept across the other matrix surfaces (PO matrix, ATS, allocations).
