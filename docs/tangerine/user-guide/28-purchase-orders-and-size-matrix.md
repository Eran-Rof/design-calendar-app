...
> **All money columns are PER-EACH (grain rule).** Avg PO Price, Avg cost and Sell price are each a **`Σ dollars ÷ Σ eaches`** average, so a PPK-prepack PO reads at the same per-garment grain as a loose PO and the three are directly comparable. The trap this avoids: a **PPK line** stores its qty as **packs** and its cost as **per-pack**, and the cost/sell reference tables (`ip_item_avg_cost`, sales history) store PPK values **per-pack too** (a PPK60 pack's cost *is* $324). Weighting a per-pack value by *eaches* would read that $324 as **$324/each** (×60 inflation) and blow up the margin (~98%). The server normalises every value to per-each in one shared, unit-tested helper (`computePoLineMoney`), weighting native-grain (per-pack) values by qty and garment-list (per-each) values by eaches — so the PPK grain bug can't reappear column-by-column.
