...
### Why one size showed as two half-empty columns (fixed)

The same story plays out along the **size** axis. The catalog stores one logical size in different spellings **across the colorways of a single style** — on PO ROF-P000510, style `RYB1500` had **Black** stored as `XLG` while **Grey**/**Shiitake** were stored as `XL`. Keyed on the raw label, the grid rendered **both an `XL` and an `XLG` column**, each half-empty. Every PO / SO / AR size grid — and the printable/Excel document — now **merges size columns by their canonical size**, so `XL` and `XLG` land in **one** column (qty and cost summed) exactly as `S`/`SML`, `M`/`MED`, `L`/`LRG` do. The merged column is labelled with the **house form** you actually use — **`SML` / `MED` / `LRG` / `XLG`** (and `XXL`, `XS`) — never the internal scale token (`SMALL`/`XLARGE`). Numeric waists (`28`–`38`), kids age-range sizes (`XS(5-6)`), and PPK pack tokens (`PPK24`) pass through untouched. The green **⋯ collapse** toggle still hides the empty leading/trailing columns on the merged grid. Presentation only — no catalog SKU is rewritten.
