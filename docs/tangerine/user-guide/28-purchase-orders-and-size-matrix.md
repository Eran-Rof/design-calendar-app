...
1. **New PO** → fill the **rich document header**, grouped into bordered boxes (the boxes have no titles; all lookups show the **name only**, no codes):
   ...
   - **Dates** — two rows: row 1 = order, **Requested in DC**, port, expected; row 2 = ship-window start, ship-window end, cancel, vendor-confirmed / acknowledged. The header **Requested in DC** date pre-fills each style's per-line date (see the matrix notes below).
...
2. **Add lines — the body IS the size matrix** (the same `LineMatrixBody` the Sales Order modal uses, in `mode="po"`). It opens with the matrix ready:
   ...
   - **🖨 View** (next to Close) opens the PO as a **branded printable document** in a new window — Ring of Fire logo, header fields (vendor, dates, terms, ship-to…), then a **color × size matrix per style** (one block per style: color/inseam rows × size columns, with a per-row Qty, Unit Cost $ and line total, plus a per-style totals row) and grand totals. Use the **Print / Save as PDF** button in that window to print or download. Works for a draft too (shows `(draft)` for the number), reflecting whatever is currently entered.
...
- **Merge PPK** (Snapshot only) — collapses each base style **and its PPK sibling** into one row labelled **`BASE/PPK`** (e.g. `RYB0594` + `RYB0594PPK` → **`RYB0594/PPK`**), **per colour** (one merged row per colour). Selecting it **auto-enables Explode** (so the merged numbers are in eaches). Only bases that actually have a PPK sibling merge; every other style is untouched. Merged rows are **tinted purple** (with a purple style code) so they stand out, and their quantities are plain (an aggregate of two styles has no single drill target).
- **Collapse onto** (Snapshot only, the dropdown next to **Columns**) — pick which **text column** to collapse *onto*: **Color** or **Item Category**. The checked column becomes the group-by key; **every other text column folds away** and the numeric columns are **summed** (Avg Cost averaged). So *Collapse onto Item Category* gives **one line per category** — all of the same category merged into a single row regardless of style, colour or name, showing just the category name (style/colour/name show **"—"**). *Collapse onto Color* gives one row per colour; check both for one row per category + colour. The button turns blue when a collapse is active and closes on mouse-off. (Independent of column show/hide.) *Style and Name were removed as options — they aren't useful group-bys (#1442). Earlier the control worked the opposite way (a checked column was removed from the key); it now reads literally as "collapse onto X".*
- **Totals** (Snapshot only, the **Qty / $** selector on the controls row) — modelled on the ATS totals view. Pick **Qty** or **$** to show a totals strip **above the column headers** summing every quantity column: **Qty** = unit counts, **$** = quantity × **avg unit cost** (Avg Cost itself isn't totalled). Click the active mode again to turn totals off. The strip is sticky and reflects the rows currently shown (after Hide Zeros / Collapse / Merge PPK), so the totals always match the visible table (#1443).
