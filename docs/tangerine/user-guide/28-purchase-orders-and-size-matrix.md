...
2. **Add lines — the body IS the size matrix** (the same `LineMatrixBody` the Sales Order modal uses, in `mode="po"`). It opens with the matrix ready:
   ...
   - **🖨 View** (next to Close) opens the PO as a **branded printable document** in a new window — Ring of Fire logo, header fields (vendor, dates, terms, ship-to…), the line items (style · color · inseam · size · qty · unit cost · total) and grand totals. Use the **Print / Save as PDF** button in that window to print or download. Works for a draft too (shows `(draft)` for the number), reflecting whatever is currently entered.
