...
**Status filter (multi-select).** The **Status** filter is a **multi-select dropdown** (mirrors the Sales Orders grid): tick **any combination** of `draft / issued / in_transit / received / cancelled` and the list shows POs in **any** of the picked statuses; leave it empty for **All statuses**. The dropdown has its own search box + a **Clear** action, and a **Clear filters** button (next to the date pickers) resets status, vendor, search and dates together. The status set is sent to the server as a comma list (`?status=issued,in_transit`) so it narrows the whole book, not just the loaded page.
...
- **Sell price** — the **qty-weighted average selling price** across the PO's lines. Sell is resolved per style: when the PO is tied to a customer (its `sales_order_id` → `sales_orders.customer_id`) the **M43 customer price** is used (own → assigned → tier → default list); otherwise it falls back to the style's **brand DEFAULT price list** (`price_lists.brand_id` = the style's brand), and finally to the SKU's **standard unit price** (`ip_item_avg_cost.standard_unit_price`, via the same resolver) — which covers styles with no customer or brand-list price. A `—` means none of those resolved.
