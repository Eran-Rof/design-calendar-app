...
- **Collapse** (Snapshot only, the dropdown next to **Columns**) — pick which **text column(s)** to roll up on: **Style, Color, Name, Item Category**. Collapsing a column merges the rows that differ only by it, **summing the numeric columns** (Avg Cost averaged), and that column shows **"—"**. So *Collapse → Color* gives **one row per style** (its colours summed); collapse Style + Color + Name and leave Item Category for **one row per category**. The button turns blue when a collapse is active and closes on mouse-off. (Independent of column show/hide.)
