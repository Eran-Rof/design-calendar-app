...
| **P15** Pricing | M43 Pricing Engine | 🟡 **engine shipped** — Chunk A: `price_lists` / `price_list_items` (qty breaks) / `price_promotions` schema, unified JS engine `api/_lib/pricing/engine.js` (precedence customer-own → assigned → tier → default list, + best promotion), `customers.price_list_id`, B2B catalog/orders repointed to the engine, resolve endpoint h578. Chunk B (#793): **Price Lists** admin panel (lists + per-style prices + qty breaks) + **Promotions** panel + customer→price-list assignment (Customer Master), new **Pricing** nav group (h579–h584). **Ahead:** Chunk C — internal SO/AR price auto-fill. (The original P15 *slot* was taken by ➕Brand Master; M43 proper is now being built.) | #792 #793 |
