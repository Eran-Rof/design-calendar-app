...
| ➕ **M50 GL Brand Allocation** (per-brand P&L accounts + %-allocation engine) | 🟡 arch + A schema + **B done** (server API + COA modal `<BrandAllocationEditor>`: brand multi-select + % editor + even-split + default + 100% validation → generates `{code}-{BRAND}` children). Remaining: **C** posting-engine split + override→update-rule (the behavior-changing core), **D** Income-Statement header/children/subtotal + hide-account-# toggle. | #665 #666 #669 #670 |
