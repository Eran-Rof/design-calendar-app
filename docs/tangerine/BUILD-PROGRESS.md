...
✅ **M10 SO** complete (P16): entry panel + API, draft→confirm→AR invoice, factor approval, multi-store split. **M18 Allocations** complete: per-SO reserve (#725) + standalone **Allocations Workbench** (#778, cross-SO priority full-fill + factor-credit gate). **M44 Carrier** complete (#726).
