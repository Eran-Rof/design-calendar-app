...
| **P16** Sales | M10 SO entry · M18 Product Allocations · M24 Showroom/Line Review · M44 Carrier | ✅ **core complete.** M10 (#698-#700) + Sales/CRM/HR batches (#701–#717) + **M18 allocations (#725)** + **M44 carrier/ship (#726)** → full draft→confirm→allocate→ship→invoice lifecycle. M24 line-review delivered as **matrix SO entry**. **Matrix initiative (#727–#733):** Size Scale master, color/size/inseam matrix on inventory view + SO entry + adjustments, **M11 native Purchase Orders** (draft→issued→in_transit→received), shared style-matrix/auto-SKU lib + scale-ordered columns. | #698–#733 |
