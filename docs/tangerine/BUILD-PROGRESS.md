...
| **P13** Procurement | M11 PO origination · M38 Receiving · M26 QC · M48 Trade Compliance | 🟡 **schema + M11 PO panel only** — P13-1/2 schema (all procurement tables applied, dormant) + M11 native PO panel (#732). **Receiving / QC / customs / broker / 3-way-match UI NOT built yet** (the prior "#548 UI shipped" note was stale). **Completion program in flight (C0–C5):** C0 (#797) reconciles the two PO systems — dual receipt + commitment FK so receiving attaches to EITHER a native `purchase_orders` PO or a mirrored `tanda_pos` PO; open-PO `po_commitments` recorded on native PO issue. Next: C1 receiving + landed-cost + bookkeeper queue. | #797 |
