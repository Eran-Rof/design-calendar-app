...
| ➕ **M50 GL Brand Allocation** (per-brand P&L accounts + %-allocation engine) | 🟡 arch + A schema + B (COA UI) + **C-engine done** (`glAllocation.js` split + `expandJeLines`; wired into manual-JE posting, gated `BRAND_SCOPE_MODE=enforce`, splits balanced to brand-child accounts — no RPC change). Remaining: **C-2** AP-invoice wiring + override→update-rule UI, **D** Income-Statement header/children/subtotal + hide-account-# toggle. | #665 #666 #669 #670 #671 |
