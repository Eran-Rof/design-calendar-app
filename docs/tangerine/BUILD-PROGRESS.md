...
| ➕ **M50 GL Brand Allocation** (per-brand P&L accounts + %-allocation engine) | 🟡 arch + A schema + B (COA UI) + **C-engine** (`glAllocation.js` split + `expandJeLines` on manual-JE posting) + **C-2 AP-invoice split done** (`expandApExpenseLines` on AP posting — expense lines split to brand-child accounts, inventory lines pass through, rule re-sums into the CR AP line so the bill stays balanced; gated `BRAND_SCOPE_MODE=enforce`). Remaining: **D** Income-Statement header/children/subtotal + hide-account-# toggle. | #665 #666 #669 #670 #671 #674 |
