...
| ➕ **M50 GL Brand Allocation** (per-brand P&L accounts + %-allocation engine) | 🟡 arch + chunk A schema + **chunk B-server** (brand-allocation API `gl-accounts/:id/brand-allocation` h534: validate 100% + replace rule + generate/retire `{code}-{BRAND}` children + set brand_rollup) done. Remaining: B-UI (COA modal multi-select + % editor), C posting-engine split + override→update-rule, D Income-Statement + hide-account-# toggle. | #665 #666 #669 |
