...
> **Category, Sub Cat and Gender come from Tangerine's Style Master.** The grid's **Category** column is the Style Master's *Category* (e.g. SHORTS), **Sub Cat** is its *Sub Category* (e.g. CARGO SHORTS), and the **Gender** filter uses its *Gender* code (Mens / Womens / Boys / Child / Girls) — the same values you see and edit on Tangerine's Style Master screen, so re-categorizing a style there flows straight into planning (filters, build scoping and reports included) on the next load. The Gender dropdown always offers the full catalog's genders (not just the ones present in the current build), so you can scope a filtered build to a gender before any rows exist for it. Styles not yet in the Style Master fall back to the nightly Xoro attributes.
