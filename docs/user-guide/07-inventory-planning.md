...
> **Category and Sub Cat come from Tangerine's Style Master.** The grid's **Category** column is the Style Master's *Category* (e.g. SHORTS) and **Sub Cat** is its *Sub Category* (e.g. CARGO SHORTS) — the same values you see and edit on Tangerine's Style Master screen, so re-categorizing a style there flows straight into planning (filters, build scoping and reports included) on the next load. Styles not yet in the Style Master fall back to the nightly Xoro attributes.
