...
> **Build (filtered):** if you've set grid filters (customer, style, category, etc.), the Build button relabels itself **Build (filtered)** and only rebuilds the matching subset. A filtered build wipes out-of-scope rows within that scope — that is intentional. The **Style** and **Period** filters are **multi-value**: select several styles (or months) and the filtered build rebuilds *all* of them at once — so you can scope a run to just your four styles instead of the whole book. (Tip: to just *work on* a subset without rebuilding, set the Style filter and the grid shows only those rows — the rest of the build stays intact underneath.)
>
