...
> **Category, Sub Cat and Gender come from Tangerine's Style Master.** The grid's **Category** column is the Style Master's *Category* (e.g. SHORTS), **Sub Cat** is its *Sub Category* (e.g. CARGO SHORTS, DENIM SHORTS), and the **Gender** filter uses its *Gender* code (Mens / Womens / Boys / Child / Girls) — the same values you see and edit on Tangerine's Style Master screen, so re-categorizing a style there flows straight into planning (rows, filters, build scoping and reports included) on the next load. The **Category, Sub Cat and Gender dropdowns all offer the Style Master's taxonomy** and cascade: Sub Cat narrows to the chosen Category, and Gender narrows to the chosen Category + Sub Cat — so picking Category SHORTS → Sub Cat DENIM SHORTS offers exactly the genders Tangerine files under denim shorts (Mens / Boys / Girls), and no others. All three work **before a build has any rows**, so you can scope a filtered build up front. (Denim shorts live under SHORTS → DENIM SHORTS, while Category DENIM holds the jean fits.) Stock-buy (TBD) rows also display the **live** Style Master category — not the value captured when the row was created — so a Tangerine re-categorization updates existing planning rows too.
