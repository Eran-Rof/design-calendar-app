...
// The direct Postgres connection (db.<ref>.supabase.co:5432) is IPv6-only
// on new projects and often refuses IPv4 traffic. The *session* pooler
// (port 5432 on aws-X-<region>.pooler.supabase.com) accepts multi-statement
// SQL and is what we use for both migrations and seed.
function buildPoolerDbUrl(ref, password, region) {
  const encoded = encodeURIComponent(password);
  return `postgresql://postgres.${ref}:${encoded}@${region}.pooler.supabase.com:5432/postgres`;
}

// Pre-migration schema for tables that predate the migration system.
// Idempotent — safe to run against any DB.
const BOOTSTRAP_SQL = `DO $bootstrap$
BEGIN
  ...
  CREATE INDEX IF NOT EXISTS idx_tanda_pos_po_number ON tanda_pos (po_number);
...
async function startSupabase() {
  ...
  // Remote mode — non-interactive via --db-url. Requires:
  //   SUPABASE_PROJECT_REF=<ref>       (e.g. jrcnpfpopwjanwmzwmsc)
  //   SUPABASE_DB_PASSWORD=<password>  (from dashboard, or reset there)
  //   SUPABASE_DB_POOLER_REGION=<id>   (e.g. aws-1-us-east-2 — check the
  //                                     Transaction Pooler URI in dashboard)
  //   SUPABASE_SERVICE_ROLE_KEY=<key>  (for Admin API + service-role writes)
  const ref = process.env.SUPABASE_PROJECT_REF
    || (await prompt("Enter your staging Supabase project ref: "));
  if (!ref) throw new Error("SUPABASE_PROJECT_REF is required for --remote mode");

  const password = process.env.SUPABASE_DB_PASSWORD
    || (await prompt("Enter SUPABASE_DB_PASSWORD: "));
  if (!password) throw new Error("SUPABASE_DB_PASSWORD is required for --remote mode");

  const region = process.env.SUPABASE_DB_POOLER_REGION
    || (await prompt("Enter SUPABASE_DB_POOLER_REGION (e.g. aws-1-us-east-2): "));
  if (!region) throw new Error("SUPABASE_DB_POOLER_REGION is required for --remote mode");

  const dbUrl = buildPoolerDbUrl(ref, password, region);
  const apiUrl = `https://${ref}.supabase.co`;
  log(`Using remote staging project: ${apiUrl} (pooler: ${region})`);

  // Sanity-check the pooler connection before doing any work.
  try {
    runSilent(`npx supabase db query --db-url '${dbUrl}' 'SELECT 1;'`);
    ok("Pooler connection verified.");
  } catch (e) {
    throw new Error(`Could not connect to pooler at ${region} — verify password and region. (${e.message})`);
  }

  return { API_URL: apiUrl, PROJECT_REF: ref, DB_URL: dbUrl };
}

// ── Step 2: Run migrations + seed ─────────────────────────────────────────────

async function applyMigrationsAndSeed(sbStatus) {
  if (MODE === "local") {
    log("Resetting local DB (migrations + seed)…");
    run("npx supabase db reset");
    ok("Migrations + seed.sql applied.");
    return;
  }

  const dbUrl = sbStatus.DB_URL;
  if (!dbUrl) throw new Error("Remote mode requires DB_URL on sbStatus");

  log("Applying pre-migration bootstrap (tanda_pos + app_data)…");
  // Write BOOTSTRAP_SQL to a temp file so `db query -f` can consume it
  // as a single DO-block statement (session pooler accepts).
  const bootstrapPath = resolve(ROOT, ".staging-bootstrap.sql");
  writeFileSync(bootstrapPath, BOOTSTRAP_SQL);
  try {
    run(`npx supabase db query --db-url '${dbUrl}' -f '${bootstrapPath}'`);
  } finally {
    try { execSync(`rm -f '${bootstrapPath}'`, { stdio: "ignore" }); } catch { /* ignore */ }
  }
  ok("Bootstrap complete.");

  log("Pushing migrations to remote…");
  // `yes Y` auto-confirms the "Apply N migrations?" prompt.
  run(`yes Y | npx supabase db push --db-url '${dbUrl}' --include-all`);
  ok("All migrations applied.");

  log("Applying seed…");
  // seed.sql uses psql \ir directives; db query doesn't expand them. Strip
  // and apply each referenced file separately.
  const seedMain = readFileSync(resolve(ROOT, "supabase/seed.sql"), "utf8");
  const seedMainClean = seedMain.split("\n").filter(l => !l.startsWith("\\ir ")).join("\n");
  const seedMainPath = resolve(ROOT, ".staging-seed-main.sql");
  writeFileSync(seedMainPath, seedMainClean);
  try {
    run(`npx supabase db query --db-url '${dbUrl}' -f '${seedMainPath}'`);
  } finally {
    try { execSync(`rm -f '${seedMainPath}'`, { stdio: "ignore" }); } catch { /* ignore */ }
  }
  ok("Main seed applied.");

  for (const phase of [1, 2, 3]) {
    const fixturePath = resolve(ROOT, `supabase/seed/inventory_planning_phase${phase}_fixtures.sql`);
    const body = readFileSync(fixturePath, "utf8");
    // Wrap multi-statement fixtures in a DO block so the pooler's
    // prepared-statement path accepts them as one call.
    const wrapped = `DO $seed_phase${phase}$\nBEGIN\n${body}\nEND\n$seed_phase${phase}$;\n`;
    const tmp = resolve(ROOT, `.staging-seed-phase${phase}.sql`);
    writeFileSync(tmp, wrapped);
    try {
      run(`npx supabase db query --db-url '${dbUrl}' -f '${tmp}'`);
    } finally {
      try { execSync(`rm -f '${tmp}'`, { stdio: "ignore" }); } catch { /* ignore */ }
    }
    ok(`Inventory Planning phase${phase} fixtures applied.`);
  }
}
...
async function main() {
  ...
  try {
    ...
    if (!RESET_ONLY) {
      await applyMigrationsAndSeed(sbStatus);
    } else {
