...
//
// Tangerine — Buyer Scope Master (#1156).
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (!data) return res.status(404).json({ error: "Scope not found" });
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await admin
      ...
      .single();
    if (error) {
      if (error.code === "PGRST116") return res.status(404).json({ error: "Scope not found" });
      if (error.code === "23505") return res.status(409).json({ error: "A scope with that code already exists" });
  ...
  if (req.method === "DELETE") {
    const { count } = await admin
      ...
      .eq("scope_id", id);
    if ((count || 0) > 0) {
      return res.status(409).json({ error: `Scope is assigned to ${count} buyer(s); deactivate it (is_active=false) instead of deleting, or remove it from those buyers first.` });
    }
    const { data, error } = await admin
      .from("buyer_scope_master")
      .delete()
