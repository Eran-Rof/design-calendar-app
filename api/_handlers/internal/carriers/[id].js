// api/internal/carriers/[id]
//
// GET    — fetch a single carrier_master row.
// PATCH  — update mutable fields. `code` and `entity_id` are LOCKED
//          post-creation. Mutable: name, carrier_type, tracking_url_template,
//          sort_order, is_active.
// DELETE — hard-delete (no reference check — carrier is stored as free text
//          in sales_order_shipments.carrier; historical shipments are unaffected).
//
...
const MUTABLE_FIELDS = new Set(["name", "carrier_type", "tracking_url_template", "sort_order", "is_active"]);
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (!data) return res.status(404).json({ error: "Carrier not found" });
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await admin
      ...
      .single();
    if (error) {
      if (error.code === "PGRST116") return res.status(404).json({ error: "Carrier not found" });
  ...
  if (req.method === "DELETE") {
    // carrier is stored as free text in sales_order_shipments.carrier — no FK,
    // so historical shipments are unaffected by deletion.
    const { data, error } = await admin
      .from("carrier_master")
      .delete()
      .eq("id", id)
...
export function validatePatch(body) {
  ...
  if ("carrier_type" in out) {
    const t = String(out.carrier_type).trim();
  ...
  if ("tracking_url_template" in out) {
    out.tracking_url_template = out.tracking_url_template ? String(out.tracking_url_template).trim() || null : null;
  }
