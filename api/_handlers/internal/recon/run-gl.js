// api/internal/recon/run-gl
//
// Tangerine P9-5 — Manual trigger for the GL reconciliation engine.
// Architecture: docs/tangerine/P9-parallel-run-architecture.md §3.5 + §4.3.
//
// POST /api/internal/recon/run-gl
//   body: {
...
//   }
//
// Looks up the default entity (ROF), invokes runGlReconciliation, and
// returns the engine summary verbatim (recon_run_id + status + totals).
//
// Wired into:
//   - Wave-B weekly cron (/api/cron/recon-gl-weekly, later chunk)
//   - Wave-C "🔁 Parallel Run" admin panel "Re-run GL" button
...
export default async function handler(req, res) {
  ...
  let summary;
  try {
    summary = await runGlReconciliation({
      admin,
      entity_id: entity.id,
    ...
    return res.status(500).json({
      error: `runGlReconciliation threw: ${err?.message || String(err)}`,
    });
  }
