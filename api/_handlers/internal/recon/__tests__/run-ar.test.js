// Tangerine P9-3 — tests for the run-ar handler.
//
// Coverage:
...
vi.mock("../../../../_lib/recon/ar-engine.js", () => ({
  runArReconciliation: (...args) => engineSpy(...args),
}));
...
beforeEach(async () => {
  ...
  vi.resetModules();
  const mod = await import("../run-ar.js");
  handler = mod.default;
});
...
describe("run-ar handler — method gating", () => {
...
describe("run-ar handler — body validation", () => {
...
describe("run-ar handler — entity header", () => {
...
describe("run-ar handler — env + engine error", () => {
  it("500 when supabase env vars are not configured", async () => {
    delete process.env.VITE_SUPABASE_URL;
    vi.resetModules();
    const { default: fresh } = await import("../run-ar.js");
    engineSpy.mockResolvedValue({ ok: true, recon_run_id: "x", status: "clean", summary: {}, errors: [] });
    const res = makeRes();
    await fresh(
      makeReq({
  ...
  it("500 when engine returns ok=false", async () => {
    engineSpy.mockResolvedValue({
      ...
      status: "error",
      summary: {},
      errors: [{ kind: "tangerine_read_failed", message: "boom" }],
    });
    const res = makeRes();
    await handler(
      makeReq({
    ...
    expect(res.body.errors[0].kind).toBe("tangerine_read_failed");
  });
