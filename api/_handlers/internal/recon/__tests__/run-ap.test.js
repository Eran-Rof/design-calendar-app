// Tangerine P9-2 — tests for /api/internal/recon/run-ap handler.
//
...
vi.mock("../../../../_lib/recon/ap-engine.js", () => ({
  runApReconciliation: vi.fn(),
}));

import handler, { validateBody } from "../run-ap.js";
import { runApReconciliation } from "../../../../_lib/recon/ap-engine.js";
...
describe("POST /api/internal/recon/run-ap", () => {
  ...
  it("200 with engine summary on happy path", async () => {
    runApReconciliation.mockResolvedValue({
      recon_run_id: "rrr-1",
      status: "clean",
      rows_compared: 5,
      variances_found: 0,
    ...
    expect(runApReconciliation).toHaveBeenCalledWith(
  ...
  it("OPTIONS short-circuit (CORS preflight) → 200 no body", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(runApReconciliation).not.toHaveBeenCalled();
  });
  ...
  it("500 when engine throws", async () => {
    runApReconciliation.mockRejectedValue(new Error("engine boom"));
    const res = makeRes();
    await handler(makeReq(), res);
    expect(res.statusCode).toBe(500);
    expect(res.body.error).toMatch(/runApReconciliation threw.*engine boom/);
  });
  ...
  it("propagates engine 'variance' status to response", async () => {
    runApReconciliation.mockResolvedValue({
      recon_run_id: "rrr-2",
      status: "variance",
      rows_compared: 12,
      variances_found: 3,
      total_variance_cents: 25000,
      totals_jsonb: { rows_compared: 12, variances_found: 3 },
      errors: [],
    });
    const res = makeRes();
    await handler(makeReq(), res);
    ...
    expect(res.body.total_variance_cents).toBe(25000);
  });
});
