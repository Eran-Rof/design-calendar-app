// api/internal/recon/run-ap
//
// Tangerine P9-2 — Manual trigger for the AP reconciliation engine.
// Architecture: docs/tangerine/P9-parallel-run-architecture.md §3.2 + §4.1.
//
// POST /api/internal/recon/run-ap
//   body: {
...
//   }
//
// Looks up the default entity (ROF), invokes runApReconciliation, and
// returns the engine summary verbatim (recon_run_id + status + totals).
//
// Wired into:
//   - Wave-B weekly cron (/api/cron/recon-ap-weekly, P9-2b)
//   - Wave-C "🔁 Parallel Run" admin panel "Re-run AP" button (P9-3)
...
export default async function handler(req, res) {
  ...
  let summary;
  try {
    summary = await runApReconciliation({
      admin,
      entity_id: entity.id,
    ...
    return res.status(500).json({
      error: `runApReconciliation threw: ${err?.message || String(err)}`,
    });
  }
