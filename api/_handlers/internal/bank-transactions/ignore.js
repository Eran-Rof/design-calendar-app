// api/internal/bank-transactions/:id/ignore
//
// POST. Body: { actor_user_id?, reason? }
// Calls bank_ignore RPC.
//
// Tangerine P6-5.
...
export function validateBody(body) {
  const out = { actor_user_id: null, reason: null };
...
export default async function handler(req, res) {
  ...
  const { data, error } = await admin.rpc("bank_ignore", {
  ...
  if (error) {
    ...
    if (/must be unmatched or matched|not found/.test(msg)) return res.status(409).json({ error: msg });
