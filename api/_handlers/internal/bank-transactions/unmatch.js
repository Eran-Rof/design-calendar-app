// api/internal/bank-transactions/:id/unmatch
//
// POST. Body: { actor_user_id?, notes? }
// Calls bank_unmatch RPC.
//
// Tangerine P6-5.
...
export function validateBody(body) {
  const out = { actor_user_id: null, notes: null };
...
export default async function handler(req, res) {
  ...
  const { data, error } = await admin.rpc("bank_unmatch", {
  ...
  if (error) {
    ...
    if (/must be matched|not found/.test(msg)) return res.status(409).json({ error: msg });
