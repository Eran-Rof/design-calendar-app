// api/internal/shopify/post-order/:id
//
// Tangerine P11-3 — manual trigger for the Shopify AR JE posting service.
//
// POST /api/internal/shopify/post-order/:id
//   :id = shopify_orders.id (uuid).
//
// Body (optional, ignored today; reserved for future flags).
//
// Calls postShopifyOrderJe({ shopifyOrderId, adminClient }). Returns:
...
//   404 { error: 'shopify_orders <id> not found' }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ar_invoice_insert_failed / shopify_orders_update_failed — JE posted
    // but stamps failed; surface as 500 so operator can inspect.
    if (e?.code === "ar_invoice_insert_failed" || e?.code === "shopify_orders_update_failed") {
