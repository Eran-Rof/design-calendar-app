// api/internal/walmart/post-order/:id
//
// Tangerine P12b-3 — manual trigger for the Walmart AR JE posting service.
//
// POST /api/internal/walmart/post-order/:id
//   :id = walmart_orders.id (uuid).
//
// Body (optional, reserved for future flags).
//
// Calls postWalmartOrderJe({ walmartOrderId, adminClient }). Returns:
...
//   401 { error: ... }                                      — auth gate
//   404 { error: 'walmart_orders <id> not found' }
//   500 { error: ... }                                      — RPC failed / unknown
//
// Auth: gated by authenticateInternalCaller (same gate as every other
// /api/internal/** handler — Bearer or X-Internal-Token; soft-open when
// INTERNAL_API_TOKEN is unset).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (e?.code === "ar_invoice_insert_failed" || e?.code === "walmart_orders_update_failed") {
