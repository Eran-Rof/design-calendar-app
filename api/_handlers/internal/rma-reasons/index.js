// api/internal/rma-reasons
//
// GET  — list RMA reasons for the default entity. By default returns
...
// POST — create one rma_reason_master row. Body:
//          { name (required),
//            sort_order (>=0, optional, default 0), is_active (default true) }
//          The `code` is SERVER-GENERATED (RMAR-NNNNN); any client-supplied
//          `code` is ignored. (Auto-coded master — operator item 14 pattern.)
//
// Tangerine — RMA Reason Master. Mirrors the seasons handler shape
// (resolveDefaultEntityId + ROF scope; service-role writes; anon-read in DB),
// minus the ordered `sizes` array — a reason is just a named code.
...
// RMA reason codes are server-generated + read-only (operator item 14): PREFIX +
// 5-digit zero-padded sequence (count existing rows carrying the prefix, +1),
// e.g. RMAR-00001.
const CODE_PREFIX = "RMAR-";
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await insertWithAutoCode(
      admin, "rma_reason_master", "code", CODE_PREFIX, buildRow, { entityId },
    );
    if (error) {
      if (error.code === "23505") {
        return res.status(409).json({ error: "Could not allocate a unique RMA reason code; please retry" });
      }
...
export function validateInsert(body) {
  ...
  // `code` is server-generated (RMAR-NNNNN); not required from the client.
