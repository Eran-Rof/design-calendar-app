// api/internal/seasons/[id]
//
// GET    — fetch a single season_master row.
...
//          references this season by NAME (style_master.season is free text,
//          not an FK — we match on the name to avoid orphaning tagged styles).
//
// Tangerine — Season Master.
...
export default async function handler(req, res) {
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await admin
      ...
      .single();
    if (error) {
      if (error.code === "PGRST116") return res.status(404).json({ error: "Season not found" });
  ...
  if (req.method === "DELETE") {
    // Fetch the season's name first; style_master references seasons by NAME
    // (free text), not by FK, so we block the delete if any style still carries it.
    ...
    if ((count || 0) > 0) {
      return res.status(409).json({
        error: "Season is still referenced. Reassign or clear those styles before deleting, or toggle is_active=false instead.",
