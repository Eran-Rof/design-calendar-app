// api/internal/style-classifications/[id]
//
// GET    — fetch a single style_classifications row.
// PATCH  — update mutable fields. `kind`, `entity_id`, `id` are LOCKED
...
const LOCKED_FIELDS = new Set(["kind", "entity_id", "id"]);
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (!data) return res.status(404).json({ error: "Classification not found" });
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await admin
      ...
      .single();
    if (error) {
      if (error.code === "PGRST116") return res.status(404).json({ error: "Classification not found" });
      if (error.code === "23505") return res.status(409).json({ error: "A classification with that name already exists for this kind/entity" });
