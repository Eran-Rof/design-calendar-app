// api/internal/sales-by-rep
//
// Tangerine P7-7 — Sales by Rep × Period report.
//
// GET — return per-rep aggregates (invoice_count, gross_cents, commission_cents)
// for a date window via RPC sales_by_rep(p_entity_id, p_from, p_to).
//
// Query params:
...
//
// Response shape:
//   { from, to, rows: [{ sales_rep_id, sales_rep_name, invoice_count,
//     gross_cents, commission_cents }] }
...
export default async function handler(req, res) {
  ...
  try {
    const { data, error } = await admin.rpc("sales_by_rep", {
    ...
    const rows = (data || []).slice().sort(
      (a, b) => Number(b.gross_cents || 0) - Number(a.gross_cents || 0),
    );
