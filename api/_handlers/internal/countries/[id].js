// api/internal/countries/[id]
//
// GET    — fetch a single country_master row.
// PATCH  — update mutable fields. `iso2` and `id` are LOCKED post-creation.
//          Mutable: name, sort_order, is_active.
// DELETE — hard-delete.
//
// Chunk I — Country Master. country_master is global (no entity scope).
...
const LOCKED_FIELDS = new Set(["iso2", "id"]);
...
export default async function handler(req, res) {
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await admin
      ...
      .single();
    if (error) {
      if (error.code === "PGRST116") return res.status(404).json({ error: "Country not found" });
