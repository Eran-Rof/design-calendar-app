// api/internal/transfer-reasons
//
// GET  — list transfer reasons for the default entity. By default returns
...
// POST — create one transfer_reason_master row. Body:
//          { name (required),
//            sort_order (>=0, optional, default 0), is_active (default true) }
//          The `code` is SERVER-GENERATED (XFRR-NNNNN); any client-supplied
//          `code` is ignored. (Auto-coded master — operator item 14 pattern.)
//
// Tangerine — Transfer Reason Master. Mirrors the adjustment-types / rma-reasons
// handler shape (resolveDefaultEntityId + ROF scope; service-role writes;
// anon-read in DB). This master is an inventory-transfer REASON/category only —
// it does NOT drive accounting; the chosen reason NAME is stored on
// inventory_transfers.notes as free text (no FK).
...
// Transfer reason codes are server-generated + read-only (operator item 14):
// PREFIX + 5-digit zero-padded sequence (count existing rows carrying the
// prefix, +1), e.g. XFRR-00001.
const CODE_PREFIX = "XFRR-";
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await insertWithAutoCode(
      admin, "transfer_reason_master", "code", CODE_PREFIX, buildRow, { entityId },
    );
    if (error) {
      if (error.code === "23505") {
        return res.status(409).json({ error: "Could not allocate a unique transfer reason code; please retry" });
      }
...
export function validateInsert(body) {
  ...
  // `code` is server-generated (XFRR-NNNNN); not required from the client.
