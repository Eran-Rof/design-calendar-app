// api/internal/adjustment-types
//
// GET  — list adjustment types for the default entity. By default returns
...
// POST — create one adjustment_type_master row. Body:
//          { name (required),
//            sort_order (>=0, optional, default 0), is_active (default true) }
//          The `code` is SERVER-GENERATED (ADJT-NNNNN); any client-supplied
//          `code` is ignored. (Auto-coded master — operator item 14 pattern.)
//
// Tangerine — Adjustment Type Master. Mirrors the rma-reasons handler shape
// (resolveDefaultEntityId + ROF scope; service-role writes; anon-read in DB).
// This master is an inventory-adjustment CATEGORY/reason only — it does NOT
// drive FIFO accounting; inventory_adjustments.adjustment_type stores the chosen
// type NAME as free text (no FK).
...
// Adjustment type codes are server-generated + read-only (operator item 14):
// PREFIX + 5-digit zero-padded sequence (count existing rows carrying the
// prefix, +1), e.g. ADJT-00001.
const CODE_PREFIX = "ADJT-";
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await insertWithAutoCode(
      admin, "adjustment_type_master", "code", CODE_PREFIX, buildRow, { entityId },
    );
    if (error) {
      if (error.code === "23505") {
        return res.status(409).json({ error: "Could not allocate a unique adjustment type code; please retry" });
      }
...
export function validateInsert(body) {
  ...
  // `code` is server-generated (ADJT-NNNNN); not required from the client.
