// api/internal/adjustment-reasons/[id]
//
// GET    — fetch a single adjustment_reason_master row.
// PATCH  — update mutable fields. `code` and `entity_id` are LOCKED
//          post-creation. Mutable: name, sort_order, is_active.
// DELETE — hard-delete. No reference check needed since reason is stored
//          as free text on inventory_adjustments.reason (no FK).
//
// Tangerine — Adjustment Reason Master.
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (!data) return res.status(404).json({ error: "Adjustment reason not found" });
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await admin
      ...
      .single();
    if (error) {
      if (error.code === "PGRST116") return res.status(404).json({ error: "Adjustment reason not found" });
