// api/internal/adjustment-reasons
//
// GET  — list adjustment reasons for the default entity. By default returns
...
// POST — create one adjustment_reason_master row. Body:
//          { name (required),
//            sort_order (>=0, optional, default 0), is_active (default true) }
//          The `code` is SERVER-GENERATED (ADJR-NNNNN); any client-supplied
//          `code` is ignored. (Auto-coded master — operator item 14 pattern.)
//
// Tangerine — Adjustment Reason Master. Mirrors the rma-reasons handler shape.
...
// Adjustment reason codes are server-generated + read-only: PREFIX +
// 5-digit zero-padded sequence, e.g. ADJR-00001.
const CODE_PREFIX = "ADJR-";
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await insertWithAutoCode(
      admin, "adjustment_reason_master", "code", CODE_PREFIX, buildRow, { entityId },
    );
    if (error) {
      if (error.code === "23505") {
        return res.status(409).json({ error: "Could not allocate a unique adjustment reason code; please retry" });
      }
...
export function validateInsert(body) {
  ...
  // `code` is server-generated (ADJR-NNNNN); not required from the client.
