...
export default async function handler(req, res) {
  ...
  // ── Pass 1: mark expired ─────────────────────────────────────────────
  try {
    // Snapshot docs about to be flipped so we can write audit rows
    const { data: willExpire } = await admin
    ...
    for (const d of willExpire || []) {
      await writeAudit(admin, {
        vendor_id: d.vendor_id, document_id: d.id, action: "expired",
  ...
  const { emails: internalEmails } = await resolveInternalRecipients(admin, "compliance", { event: "compliance_expiring_soon" });
