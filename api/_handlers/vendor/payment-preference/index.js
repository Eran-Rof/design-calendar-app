// api/vendor/payment-preference
//
// Spec-aligned singular endpoint. GET/PUT are the same shape as the plural
// /api/vendor/payment-preferences endpoint; PUT here behaves like POST there.
...
export default async function handler(req, res) {
  ...
  if (req.method === "PUT") {
    ...
    const { data: inserted, error } = await admin.from("vendor_payment_preferences").insert({ vendor_id: vendorId, ...payload }).select("id").single();
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ id: inserted?.id, upserted: "inserted" });
  }
