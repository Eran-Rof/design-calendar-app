// api/xoro/writeback/reserve-update.js
//
// Covers reserve_inventory / release_reserve / update_protection_qty —
// the payload's `operation` field disambiguates.
...
export default async function handler(req, res) {
  ...
  const missing = requireFields(data, ["sku_id", "qty", "operation"]);
  if (missing) {
  ...
  if (!["reserve", "release", "set_protection"].includes(data.operation)) {
    const f = failResult({ action_id, dry_run, message: `Invalid operation: ${data.operation}`, status: 400 });
    await auditWriteback({ actionId: action_id }, "dry_run", f.body);
    return res.status(f.status).json(f.body);
  }

  const preview = placeholderResponse("reserve_update", data);
