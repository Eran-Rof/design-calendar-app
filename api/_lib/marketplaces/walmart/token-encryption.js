...
// Same algorithm as Plaid (api/_lib/plaid/encryption.js, key =
// PLAID_TOKEN_ENC_KEY) from P6-2 and Shopify (api/_lib/shopify/
// token-encryption.js, key = SHOPIFY_TOKEN_ENC_KEY) from P11-2 —
...
//
// Tangerine P12b-2 — real AES-256-GCM impl. Replaces the P12b-1 stub.
...
function getKey() {
  const hex = process.env.WALMART_TOKEN_ENC_KEY;
  if (!hex || typeof hex !== "string") {
    throw new Error("WALMART_TOKEN_ENC_KEY env var is required (64 hex chars / 32 bytes)");
  }
  const clean = hex.trim();
  if (!/^[0-9a-fA-F]{64}$/.test(clean)) {
    throw new Error("WALMART_TOKEN_ENC_KEY must be exactly 64 hex characters (32 bytes AES-256 key)");
  }
 ...
 * are nullable in the schema (a seller account row can exist before its
 ...
 * WALMART_TOKEN_ENC_KEY is present and keys are wired correctly.
 */
export function selfCheck() {
  // Synthetic test value — NOT a real Walmart credential. Format is
  // intentionally NOT the live Walmart prefix to keep GH secret scanning
  // from flagging this string.
  const sample = "test-roundtrip-sample-not-a-real-credential";
  const { ciphertext, iv, tag } = encryptToken(sample);
  const round = decryptToken(ciphertext, iv, tag);
  if (round !== sample) {
    throw new Error("Walmart token encryption self-check failed (roundtrip mismatch)");
  }
